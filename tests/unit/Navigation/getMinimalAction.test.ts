...
describe('getMinimalAction', () => {
    ...
    it('pushes a new split navigator when the focused split has no sidebar screen', () => {
        // On a narrow layout SplitRouter omits the sidebar screen (`shouldSplitHaveSidebar`), so the scope of the
        // split has to be read from its focused central screen instead.
        const splitState = buildNavigationState('split-state', 1, [
            {key: 'central-route-1', name: SCREENS.WORKSPACE.MEMBERS, params: {policyID: POLICY_B}},
            {key: 'central-route-2', name: SCREENS.WORKSPACE.PROFILE, params: {policyID: POLICY_B}},
        ]);
        const workspaceState = buildNavigationState('workspace-state', 0, [{key: 'split-route', name: NAVIGATORS.WORKSPACE_SPLIT_NAVIGATOR, state: splitState}]);
        const tabState = buildNavigationState('tab-state', 0, [{key: 'workspace-route', name: NAVIGATORS.WORKSPACE_NAVIGATOR, state: workspaceState}]);
        const rootState = buildNavigationState('root-state', 0, [{key: 'tab-route', name: NAVIGATORS.TAB_NAVIGATOR, state: tabState}]);

        expect(getMinimalAction(buildWorkspaceAction(POLICY_A), rootState).action).toMatchObject({
            type: 'PUSH',
            target: 'workspace-state',
            payload: {
                name: NAVIGATORS.WORKSPACE_SPLIT_NAVIGATOR,
                params: {screen: SCREENS.WORKSPACE.MORE_FEATURES, params: {policyID: POLICY_A}},
            },
        });
    });
...
describe('split navigator scope', () => {
    it('is defined only for the splits whose sidebar screen has a path parameter', () => {
        // The scope is derived from the linking config rather than hardcoded, so this pins which splits it currently
        // covers. Adding a scoped split navigator, or adding a path param to an existing sidebar, must fail here.
        const scopeParamsByNavigator = Object.fromEntries(
            Object.values(SIDEBAR_TO_SPLIT).map((splitNavigatorName) => [splitNavigatorName, getParamsFromRoute(SPLIT_TO_SIDEBAR[splitNavigatorName])]),
        );

        expect(scopeParamsByNavigator).toEqual({
            [NAVIGATORS.SETTINGS_SPLIT_NAVIGATOR]: [],
            [NAVIGATORS.REPORTS_SPLIT_NAVIGATOR]: [],
            [NAVIGATORS.WORKSPACE_SPLIT_NAVIGATOR]: ['policyID'],
            [NAVIGATORS.DOMAIN_SPLIT_NAVIGATOR]: ['domainAccountID'],
        });
    });
});
