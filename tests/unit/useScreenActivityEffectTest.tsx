...
describe('useScreenActivityEffect', () => {
    ...
    it('runs the cleanup at once when the component is removed while the screen is visible', () => {
        const {rerender, unmount} = render(<Screen mode="visible" />);
        log.length = 0;
        rerender(
            <Screen
        ...
        // The boundary gave the entry back, so the terminal release has nothing left to run for it.
        unmount();
        expect(log).toEqual(['cleanup:a']);
    });

    it('runs the cleanup at once when the component is removed after a cover and reveal cycle', () => {
        const {rerender} = render(<Screen mode="visible" />);
        rerender(<Screen mode="hidden" />);
        rerender(<Screen mode="visible" />);
        log.length = 0;
    ...
    it('runs the cleanup of a component removed while the screen was hidden on the next reveal', () => {
        const {rerender, unmount} = render(<Screen mode="visible" />);
        rerender(<Screen mode="hidden" />);
        log.length = 0;
        rerender(
        ...
        // The sweep gave the entry back, so the terminal release has nothing left to run for it.
        unmount();
        expect(log).toEqual(['cleanup:a']);
    });
    ...
    it('runs the cleanup for the StrictMode remount cycle that runs below the boundary', () => {
        render(
            <Screen
                mode="visible"
                isStrict
