...
| Slice | Release | p95 | n | this release's share of the platform |
|---|---|---|---|---|
...
Each share says how much of that platform's traffic ran on that one release: the iOS rows are measured against iOS traffic (42K measurements) and the Android rows against Android traffic (10K). Within one platform the shares add up to at most 100%; the rest is versions below the floor.
...
Each share says how much of that platform's traffic ran on that one release: the iOS rows are measured against iOS traffic (33K measurements) and the Android rows against Android traffic (9,134). Within one platform the shares add up to at most 100%; the rest is versions below the floor.
...
Each share says how much of that platform's traffic ran on that one release: the iOS rows are measured against iOS traffic (14K measurements) and the Android rows against Android traffic (4,750). Within one platform the shares add up to at most 100%; the rest is versions below the floor.
...
Each share says how much of that platform's traffic ran on that one release: the iOS rows are measured against iOS traffic (334K measurements) and the Android rows against Android traffic (88K). Within one platform the shares add up to at most 100%; the rest is versions below the floor.
...
Each share says how much of that platform's traffic ran on that one release: the iOS rows are measured against iOS traffic (89K measurements) and the Android rows against Android traffic (36K). Within one platform the shares add up to at most 100%; the rest is versions below the floor.
...
Each share says how much of that platform's traffic ran on that one release: the iOS rows are measured against iOS traffic (1,848 measurements) and the Android rows against Android traffic (956). Within one platform the shares add up to at most 100%; the rest is versions below the floor.
...
Each share says how much of that platform's traffic ran on that one release: the iOS rows are measured against iOS traffic (6,099 measurements) and the Android rows against Android traffic (1,945). Within one platform the shares add up to at most 100%; the rest is versions below the floor.
...
Each share says how much of that platform's traffic ran on that one release: the iOS rows are measured against iOS traffic (2,036 measurements) and the Android rows against Android traffic (605). Within one platform the shares add up to at most 100%; the rest is versions below the floor.
...
Each share says how much of that platform's traffic ran on that one release: the iOS rows are measured against iOS traffic (107K measurements) and the Android rows against Android traffic (42K). Within one platform the shares add up to at most 100%; the rest is versions below the floor.
...
Each share says how much of that platform's traffic ran on that one release: the iOS rows are measured against iOS traffic (125K measurements) and the Android rows against Android traffic (45K). Within one platform the shares add up to at most 100%; the rest is versions below the floor.
...
Each share says how much of that platform's traffic ran on that one release: the iOS rows are measured against iOS traffic (2,912 measurements) and the Android rows against Android traffic (531). Within one platform the shares add up to at most 100%; the rest is versions below the floor.
