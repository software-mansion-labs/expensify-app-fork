                ...
                Object.entries(reports).forEach(([reportID, report]) => {
                    ...
                    promises.push(
                        Promise.all([
                            ...
                            void Onyx.merge(reportID, {pendingChatMembers: null}),
                            // eslint-disable-next-line rulesdir/prefer-actions-set-data
                            void Onyx.merge(`${ONYXKEYS.COLLECTION.REPORT_METADATA}${report.reportID}`, {pendingChatMembers: report.pendingChatMembers}),
                        ]).then(() => {
