...
function setSupportAuthToken(supportAuthToken: string, email: string, accountID: number) {
    ...
    void Onyx.set(ONYXKEYS.LAST_VISITED_PATH, '');
}
            ...
            if (response?.hasOldDotAuthCookies) {
                ...
                asyncOpenURL(
                    redirectToSignIn().then(() => {
                        void Onyx.multiSet(onyxSetParams);
                    }),
...
function resendValidateCode(login = credentials.login) {
    ...
    void API.write(WRITE_COMMANDS.REQUEST_NEW_VALIDATE_CODE, params, {optimisticData, finallyData});
}
...
function signUpUser() {
    ...
    void API.write(WRITE_COMMANDS.SIGN_UP_USER, params, {optimisticData, successData, failureData});
}
...
function beginAppleSignIn(idToken: string | undefined | null) {
    ...
    void API.write(WRITE_COMMANDS.SIGN_IN_WITH_APPLE, params, {optimisticData, successData, failureData});
}
...
function beginGoogleSignIn(token: string | null) {
    ...
    void API.write(WRITE_COMMANDS.SIGN_IN_WITH_GOOGLE, params, {optimisticData, successData, failureData});
}
...
function signIn(validateCode: string, twoFactorAuthCode?: string) {
    ...
    Device.getDeviceInfoWithID().then((deviceInfo) => {
        ...
        void API.write(WRITE_COMMANDS.SIGN_IN_USER, params, {optimisticData, successData, failureData});
    });
}
...
function signInWithValidateCode(accountID: number, code: string, twoFactorAuthCode = '') {
    ...
    Device.getDeviceInfoWithID().then((deviceInfo) => {
        ...
        void API.write(WRITE_COMMANDS.SIGN_IN_USER_WITH_LINK, params, {optimisticData, successData, failureData});
    });
}
...
function invalidateCredentials() {
    void Onyx.merge(ONYXKEYS.CREDENTIALS, {autoGeneratedLogin: '', autoGeneratedPassword: ''});
}

function invalidateAuthToken() {
    NetworkStore.setAuthToken(INVALID_TOKEN);
    void Onyx.merge(ONYXKEYS.SESSION, {authToken: INVALID_TOKEN, encryptedAuthToken: INVALID_TOKEN});
}
...
function expireSessionWithDelay() {
    ...
    setTimeout(() => {
        NetworkStore.setAuthToken(INVALID_TOKEN);
        void Onyx.merge(ONYXKEYS.SESSION, {authToken: INVALID_TOKEN, encryptedAuthToken: INVALID_TOKEN, creationDate: new Date().getTime() - CONST.SESSION_EXPIRATION_TIME_MS});
    }, 15000);
}
...
function clearSignInData() {
    ...
    void Onyx.merge(ONYXKEYS.HYBRID_APP, {signingInWithSAML: null});
}
...
function setAccountError(error: string) {
    void Onyx.merge(ONYXKEYS.ACCOUNT, {errors: ErrorUtils.getMicroSecondOnyxErrorWithMessage(error)});
}
...
function requestUnlinkValidationLink() {
    ...
    void API.write(WRITE_COMMANDS.REQUEST_UNLINK_VALIDATION_LINK, params, {optimisticData, successData, failureData});
}
...
function unlinkLogin(accountID: number, validateCode: string) {
    ...
    void API.write(WRITE_COMMANDS.UNLINK_LOGIN, params, {
        optimisticData,
        successData,
        failureData,
    });
...
function toggleTwoFactorAuth(enable: boolean, twoFactorAuthCode = '') {
    ...
    if (enable) {
        void API.write(WRITE_COMMANDS.ENABLE_TWO_FACTOR_AUTH, null, {optimisticData, successData, failureData});
        return;
    }
    ...
    void API.write(WRITE_COMMANDS.DISABLE_TWO_FACTOR_AUTH, params, {optimisticData, successData, failureData});
}

function clearDisableTwoFactorAuthErrors() {
    void Onyx.merge(ONYXKEYS.ACCOUNT, {errorFields: {requiresTwoFactorAuth: null}});
}
...
function validateTwoFactorAuth(twoFactorAuthCode: string, shouldClearData: boolean) {
    ...
    // eslint-disable-next-line rulesdir/no-api-side-effects-method
    void API.makeRequestWithSideEffects(SIDE_EFFECT_REQUEST_COMMANDS.TWO_FACTOR_AUTH_VALIDATE, params, {optimisticData, successData, failureData}).then((response) => {
    ...
    void API.makeRequestWithSideEffects(
        ...
        if (response?.jsonCode === CONST.JSON_CODE.EXP_ERROR) {
            ...
            if (response?.message === CONST.MERGE_ACCOUNT_INVALID_CODE_ERROR || response?.title === CONST.MERGE_ACCOUNT_INVALID_CODE_ERROR) {
                void Onyx.merge(ONYXKEYS.ONBOARDING_ERROR_MESSAGE, translateLocal('contacts.genericFailureMessages.validateSecondaryLogin'));
            } else {
                void Onyx.merge(ONYXKEYS.NVP_ONBOARDING, {isMergingAccountBlocked: true});
            }
            return;
        }
...
function resetSMSDeliveryFailureStatus(login: string) {
    ...
    void API.write(WRITE_COMMANDS.RESET_SMS_DELIVERY_FAILURE_STATUS, params, {optimisticData, successData, failureData});
}
