 ...
 */
async function signOut(params: {autoGeneratedLogin?: string; signedInWithSAML?: boolean; authToken?: string; server?: ValueOf<typeof CONST.SERVER>} = {}): Promise<void | Response<never>> {
    ...
    if (params.signedInWithSAML && params.authToken) {
        return callSAMLSignOut(logOutParams, params.authToken, params.server);
    }

    // eslint-disable-next-line rulesdir/no-api-side-effects-method
    return API.makeRequestWithSideEffects(SIDE_EFFECT_REQUEST_COMMANDS.LOG_OUT, logOutParams, {}, params.server);
}

function callSAMLSignOut(params: LogOutParams, authToken: string, server?: ValueOf<typeof CONST.SERVER>): Promise<void | Response<never>> {
    ...
    return openAuthSessionAsync(`${CONFIG.EXPENSIFY.SAML_URL}/logout?${queryString}`, expectedURL)
        ...
        .then((result) => {
            ...
            // eslint-disable-next-line rulesdir/no-api-side-effects-method
            return API.makeRequestWithSideEffects(SIDE_EFFECT_REQUEST_COMMANDS.LOG_OUT, params, {}, server);
        });
}
...
function signOutAndRedirectToSignIn(
    ...
    /**
     * The server the LogOut must reach. Only a caller that is itself changing the active server needs this —
     * routing otherwise reads the active server when the request is sent, which by then is the new one, so the
     * session on the server being left would never be invalidated.
     */
    ...
    // session. The auth token check is what keeps a signed-out caller from sending a LogOut that carries no
    // token: there is nothing to invalidate, and the request is still routed and sent. On a QA build that
    // makes it a QA request, which awaits the Cloudflare handshake and navigates the tab off whatever screen
    // the person was still using — setActiveServer signs out on every move across the QA boundary, so the
    // server switch alone was enough to trigger it.
    const signOutPromise: Promise<void | Response<never>> =
        hasAuthToken() && !shouldRestoreStashedSession && !shouldStashSession
            ? signOut({autoGeneratedLogin: credentials?.autoGeneratedLogin, signedInWithSAML: !!deprecatedSession.signedInWithSAML, authToken: deprecatedSession.authToken, server})
            : Promise.resolve();
