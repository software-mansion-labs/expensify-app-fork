...
function startOnboardingFlow(startOnboardingFlowParams: GetOnboardingInitialPathParamsType) {
    ...
    // top-level name, so once it is mounted the state cannot change - but resetRoot({stale: true}) would
    // still re-emit it and fire history.replaceState, and a burst of those trips Safari's 100-per-10s cap
    // (APP-HT5). Same bail OnboardingGuard applies to this loop (APP-7FR).
