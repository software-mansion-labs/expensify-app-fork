...
const navigateToWorkspacesPage = ({currentUserLogin, shouldUseNarrowLayout, policy, domain, lastWorkspacesTabNavigatorRoute, topmostFullScreenRoute}: Params) => {
    ...
    // If there is already a TAB_NAVIGATOR on top, both WORKSPACE_SPLIT_NAVIGATOR and
    // DOMAIN_SPLIT_NAVIGATOR can be restored by jumping to the WORKSPACE_NAVIGATOR tab in-place.
    // URL-based navigation would go through `getStateFromPath` and push a brand-new
    // TAB_NAVIGATOR on top of the existing one, which is wasteful.
    const existingTabNavStateKey = topmostFullScreenRoute?.name === NAVIGATORS.TAB_NAVIGATOR ? (topmostFullScreenRoute.state as NavigationState | undefined)?.key : undefined;
