...
import type {NavigationState, PartialState, getStateFromPath as RNGetStateFromPath, Route, findFocusedRoute} from '@react-navigation/native';
...
function getMatchingFullScreenRoute(route: NavigationPartialRoute) {
    ...
    if (route.path) {
        ...
        if (isDynamicRouteSuffix(dynamicRouteSuffix)) {
            ...
            const focusedStateForDynamicRoute = findFocusedRouteWithOnyxTabGuard(stateUnderDynamicRoute);
