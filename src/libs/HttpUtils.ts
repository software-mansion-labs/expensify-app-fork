...
import {getCfSession, markCfSessionRejected, refreshCfSession} from './actions/CloudflareSession';
...
import {isQAServerRequest} from './CloudflareOAuth/config';
...
/**
 * Inspects a parsed JSON response and maps recognized server error shapes to HttpsError rejections.
 * NOT pure — it fires `alert()` and the update-required modal — so a retried request must flow
 * through it exactly once: inside the recursive `processHTTPRequest` call, never again in the
 * outer chain (see the QA 401 retry branch below).
 */
...
function processHTTPRequest<TKey extends OnyxKey>(
    ...
    // QA server POC (see Web_POC.md): requests to the Cloudflare Access-protected QA origin — and only
    // those — carry the OAuth session as an explicit bearer header. isQAServerRequest is false for
    // every URL while the QA config is absent, so this is inert outside the POC setup.
    ...
    return (
        fetch(url, {
            ...
            .then((response) => {
                ...
                if (response.status === CONST.HTTP_STATUS.UNAUTHORIZED && qaAccessToken) {
                    if (isQARetry) {
                        ...
                        return markCfSessionRejected(qaAccessToken).then(() => {
                            throw new HttpsError({message: CONST.ERROR.CF_REAUTH_REQUIRED, status: response.status.toString()});
                        });
                    }
                    return refreshCfSession(qaAccessToken).then((refreshResult) => {
                        ...
                        // 'refreshed' / 'skipped-newer-token' → retry once with the rotated token. Transient
                        // refresh failures never reach this line: refreshCfSession rethrows them, so they
                        // propagate to the caller as ordinary network errors — NOT as the re-auth error (the
                        // session is still alive). The retried response is fully processed inside the
                        // recursion (including processJSONResponse's side effects) and must not pass through
                        // the outer chain's helper again.
            ...
            // Note: this also runs for the QA retry's outer chain (the recursion has its own finally) —
            // it no-ops for the untracked CloudflareAuthProbe command, and the probe path deliberately
            // avoids OpenApp so the addSkewList time-skew logic never sees the mock Worker's Date header.
