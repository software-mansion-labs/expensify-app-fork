...
/**
 * `activeServerState` above is only real once getEnvironment() has resolved AND the first Onyx callback below
 * has run. A one-shot decision must await this or it reads 'production' on every build, QA included.
 */
...
function getApiRoot<TKey extends OnyxKey = never>(request?: Partial<Pick<Request<TKey>, 'shouldUseSecure' | 'shouldSkipWebProxy' | 'command' | 'server'>>, forceProduction = false): string {
    ...
    const server = forceProduction ? CONST.SERVER.PRODUCTION : (request?.server ?? activeServerState.activeServer);
