...
function reauthenticate(command = ''): Promise<boolean> {
    ...
    return hasReadRequiredDataFromStorage().then(() => {
        ...
        Log.info(`reauthenticating with ${shouldUseNewPartnerName ? 'new' : 'old'} partner name`);
        if(!shouldUseNewPartnerName) {
            Log.info(`This means that user has valid credentials that were created before migration to new SignInPage`);
        }
