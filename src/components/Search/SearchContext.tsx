...
function SearchContextProvider({children}: SearchContextProps) {
    ...
    // Use navigationRef directly as fallback since this provider sits outside the navigator tree.
    const queryParam = useRootNavigationState((state) => selectSearchQueryParam(state ?? (navigationRef.isReady() ? navigationRef.getRootState() : undefined)));
    const rawQueryParam = useRootNavigationState((state) => selectSearchRawQueryParam(state ?? (navigationRef.isReady() ? navigationRef.getRootState() : undefined)));
