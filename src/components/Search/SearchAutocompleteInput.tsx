...
function SearchAutocompleteInput({
    ...
    const unstableCurrencyAutocompleteList = Object.keys(currencyList).filter((currencyCode) => !currencyList[currencyCode]?.retired);
    const currencyAutocompleteList = useStableValue(unstableCurrencyAutocompleteList);
    ...
    const [unstableAllPoliciesTags] = useOnyx(ONYXKEYS.COLLECTION.POLICY_TAGS, {selector: passthroughPolicyTagListSelector});
    const allPoliciesTags = useStableValue(unstableAllPoliciesTags);
    const unstableTagAutocompleteList = getAutocompleteTags(allPoliciesTags);
    const tagAutocompleteList = useStableValue(unstableTagAutocompleteList);
    ...
    const {borderColor: wrapperBorderColor = defaultBorderColor, ...unstableRestWrapperStyle} = wrapperStyle ?? {};
    const restWrapperStyle = useStableValue(unstableRestWrapperStyle);
