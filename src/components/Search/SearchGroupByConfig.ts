...
type SearchGroupByTitleIconName = 'Users' | 'CreditCard' | 'Send' | 'Folder' | 'Basket' | 'Tag' | 'Calendar';

type SearchGroupByConfig = {
    titleIconName: SearchGroupByTitleIconName;
    getLabel: (item: GroupedItem) => string;
    getFilterQuery: (item: GroupedItem) => string;
};

/**
 * Shared configuration for each search groupBy type - defines title icon, label extraction,
 * and filter query building for both chart and table views.
 */
const SEARCH_GROUP_BY_CONFIG: Record<SearchGroupBy, SearchGroupByConfig> = {
...
export type {SearchGroupByConfig};
export {SEARCH_GROUP_BY_CONFIG};
