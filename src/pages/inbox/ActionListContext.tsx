...
type ActionListContextType = {
    ...
    /** Each list publishes its locally-owned ref on mount. */
...
/**
 * Owns a list ref and publishes it to the context. Layout effect so it's registered at commit, before any
 * layout-time handler reads it via `getListRef()`. Returns the ref to attach.
 */
function useActionListRef() {
    const {registerListRef} = useActionListContext();
    const listRef = useRef<FlatList>(null);

    // The published value is the ref object itself and React empties it when the list detaches, so every
    // consumer already sees a null list without this hook unpublishing it on teardown.
    useLayoutEffect(() => {
