...
function getUrlWithBackToParam<TUrl extends string>(
  ...
  const backToParam = backTo
    ? (`${url.includes("?") ? "&" : "?"}backTo=${shouldEncodeURIComponent ? encodeURIComponent(backTo) : backTo}` as const)
    : "";
...
const DYNAMIC_ROUTES = {
  ...
  POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_EXPORT_OUT_OF_POCKET_EXPENSES_ACCOUNT_SELECT:
    {
      ...
      entryScreens: [
        SCREENS.WORKSPACE.ACCOUNTING
          .DYNAMIC_QUICKBOOKS_ONLINE_EXPORT_OUT_OF_POCKET_EXPENSES,
      ],
    },
  ...
  POLICY_ACCOUNTING_SAGE_INTACCT_DEFAULT_VENDOR: {
    ...
    getRoute: (reimbursable: string) =>
      getUrlWithParams("default-vendor", { reimbursable }),
  ...
  NOTIFICATION_PREFERENCES: {
    ...
    getRoute: (reportID: string) =>
      getUrlWithParams("notification-preferences", { reportID }),
...
const ROUTES = {
  ...
  SEARCH_ROOT: {
    ...
    getRoute: ({
      ...
      const rawQuerySegment = rawQuery
        ? `&rawQuery=${encodeURIComponent(rawQuery)}`
        : "";
  ...
  SEARCH_SAVED_SEARCH_RENAME: {
    ...
    }) => `search/saved-search/rename?name=${name}&q=${jsonQuery}` as const,
  },
  SEARCH_COLUMNS: "search/columns",
  SEARCH_ADVANCED_FILTERS: {
    route: "search/filters/:filterKey?/:subPage?",
    getRoute: (
      filterKey?: SearchFilterKey | UserFriendlyKey,
      subPage?: string,
    ) => {
  ...
  SEARCH_REPORT: {
    ...
    getRoute: ({
      ...
      const baseRoute = reportActionID
        ? (`search/view/${reportID}/${reportActionID}` as const)
        : (`search/view/${reportID}` as const);
  ...
  SEARCH_REPORT_VERIFY_ACCOUNT: {
    ...
    getRoute: (reportID: string) =>
      `search/view/${reportID}/${VERIFY_ACCOUNT}` as const,
  },
  ...
  SEARCH_MONEY_REQUEST_REPORT_VERIFY_ACCOUNT: {
    ...
    getRoute: (reportID: string) =>
      `search/r/${reportID}/${VERIFY_ACCOUNT}` as const,
  },
  ...
  SEARCH_MONEY_REQUEST_REPORT_REJECT_TRANSACTIONS: {
    ...
    getRoute: ({ reportID }: { reportID: string }) =>
      `search/r/${reportID}/reject` as const,
  },
  ...
  SEARCH_EDIT_MULTIPLE_TAG_RHP: {
    ...
    getRoute: (tagListIndex = 0) =>
      `search/edit-multiple/tag/${tagListIndex}` as const,
  },
  ...
  PROFILE_AVATAR: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (accountID: number, backTo?: string) =>
      getUrlWithBackToParam(`a/${accountID}/avatar` as const, backTo),
  },
  ...
  BANK_ACCOUNT_VERIFY_ACCOUNT: {
    ...
    getRoute: (policyID?: string, backTo?: string) =>
      getUrlWithBackToParam(
        `bank-account/${VERIFY_ACCOUNT}?policyID=${policyID}`,
        backTo,
      ),
  },
    ...
    getRoute: ({
      ...
      if (bankAccountID) {
        ...
        return getUrlWithBackToParam(
          `bank-account/${stepToOpen}?bankAccountID=${bankAccountID}`,
          backTo,
        );
      }
      ...
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `bank-account/${stepToOpen}?policyID=${policyID}${subStepToOpen ? `&subStep=${subStepToOpen}` : ""}`,
        backTo,
      );
    },
  },
  BANK_ACCOUNT_ENTER_SIGNER_INFO: {
    ...
    getRoute: (
      ...
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the BANK_ACCOUNT_ENTER_SIGNER_INFO route",
        );
      }
  ...
  BANK_ACCOUNT_CONNECT_EXISTING_BUSINESS_BANK_ACCOUNT: {
    ...
    getRoute: (policyID: string, backTo?: string) =>
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      getUrlWithBackToParam(
        `bank-account/connect-existing-business-bank-account?policyID=${policyID}`,
        backTo,
      ),
  },
  BANK_ACCOUNT_NON_USD_SETUP: {
    ...
    getRoute: ({
      ...
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `${base}${pagePart}${subPagePart}${actionPart}${queryString}`,
        backTo,
      );
    },
  },
  ...
  SETTINGS_PROFILE: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam("settings/profile", backTo),
  },
  ...
  SETTINGS_SUBSCRIPTION: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam("settings/subscription", backTo),
  },
  SETTINGS_SUBSCRIPTION_SIZE: {
    route: "settings/subscription/subscription-size/:subPage",
    getRoute: (subPage: string) =>
      `settings/subscription/subscription-size/${subPage}` as const,
  },
  SETTINGS_SUBSCRIPTION_SETTINGS_DETAILS: "settings/subscription/details",
  SETTINGS_SUBSCRIPTION_EXPENSIFY_CODE:
    "settings/subscription/details/expensify-code",
  ...
  SETTINGS_SUBSCRIPTION_DOWNGRADE_BLOCKED: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam("settings/subscription/downgrade-blocked", backTo),
  },
  ...
  SETTINGS_MERGE_ACCOUNTS: {
    ...
    getRoute: (email?: string) =>
      `settings/security/merge-accounts${email ? `?email=${encodeURIComponent(email)}` : ""}` as const,
  },
  SETTINGS_MERGE_ACCOUNTS_MAGIC_CODE: {
    route: "settings/security/merge-accounts/:login/magic-code",
    getRoute: (login: string) =>
      `settings/security/merge-accounts/${encodeURIComponent(login)}/magic-code` as const,
  },
  SETTINGS_MERGE_ACCOUNTS_RESULT: {
    route: "settings/security/merge-accounts/:login/result/:result",

    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (login: string, result: string, backTo?: string) =>
      getUrlWithBackToParam(
        `settings/security/merge-accounts/${encodeURIComponent(login)}/result/${result}`,
        backTo,
      ),
  },
  ...
  SETTINGS_DELEGATE_ROLE: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (login: string, role?: string, backTo?: string) =>
      getUrlWithBackToParam(
        `settings/security/delegate/${encodeURIComponent(login)}/role/${role}`,
        backTo,
      ),
  },
  SETTINGS_UPDATE_DELEGATE_ROLE: {
    route: "settings/security/delegate/:login/update-role/:currentRole",
    getRoute: (login: string, currentRole: string) =>
      `settings/security/delegate/${encodeURIComponent(login)}/update-role/${currentRole}` as const,
  },
  SETTINGS_UPDATE_DELEGATE_ROLE_CONFIRM_MAGIC_CODE: {
    route: "settings/security/delegate/:login/confirm-role/:newRole",
    getRoute: (login: string, newRole: string) =>
      `settings/security/delegate/${encodeURIComponent(login)}/confirm-role/${newRole}` as const,
  },
  SETTINGS_DELEGATE_CONFIRM: {
    route: "settings/security/delegate/:login/role/:role/confirm",
    getRoute: (login: string, role: string) =>
      `settings/security/delegate/${encodeURIComponent(login)}/role/${role}/confirm` as const,
  },
  SETTINGS_DELEGATE_CONFIRM_MAGIC_CODE: {
    route: "settings/security/delegate/:login/role/:role/confirm/magic-code",
    getRoute: (login: string, role: string) =>
      `settings/security/delegate/${encodeURIComponent(login)}/role/${role}/confirm/magic-code` as const,
  },
  ...
  SETTINGS_WALLET_PERSONAL_CARD_DETAILS: {
    ...
    getRoute: (cardID: string | undefined) => {
      if (!cardID) {
        Log.warn(
          "Invalid cardID is used to build the SETTINGS_WALLET_PERSONAL_CARD_DETAILS route",
        );
      }
  ...
  SETTINGS_WALLET_PERSONAL_CARD_EDIT_NAME: {
    ...
    getRoute: (cardID: string) =>
      `settings/wallet/personal-card/${cardID}/edit/name` as const,
  },
  SETTINGS_WALLET_PERSONAL_CARD_EDIT_TRANSACTION_START_DATE: {
    ...
    getRoute: (cardID: string) =>
      `settings/wallet/personal-card/${cardID}/edit/transaction-start-date` as const,
  },
  SETTINGS_WALLET_DOMAIN_CARD_CONFIRM_MAGIC_CODE: {
    route: "settings/wallet/card/:cardID/confirm-magic-code",
    getRoute: (cardID: string) =>
      `settings/wallet/card/${cardID}/confirm-magic-code` as const,
  },
  ...
  SETTINGS_DOMAIN_CARD_UPDATE_ADDRESS: {
    ...
    getRoute: (cardID: string) =>
      `settings/card/${cardID}/update-address` as const,
  },
  SETTINGS_DOMAIN_CARD_CONFIRM_MAGIC_CODE: {
    route: "settings/card/:cardID/confirm-magic-code",
    getRoute: (cardID: string) =>
      `settings/card/${cardID}/confirm-magic-code` as const,
  },
  ...
  SETTINGS_REPORT_FRAUD_VERIFY_ACCOUNT: {
    ...
    getRoute: (cardID: string) =>
      `settings/wallet/card/${cardID}/report-virtual-fraud/${VERIFY_ACCOUNT}` as const,
  },
  SETTINGS_REPORT_FRAUD_CONFIRMATION: {
    route: "settings/wallet/card/:cardID/report-virtual-fraud-confirm",
    getRoute: (cardID: string) =>
      `settings/wallet/card/${cardID}/report-virtual-fraud-confirm` as const,
  },
  SETTINGS_DOMAIN_CARD_REPORT_FRAUD: {
    route: "settings/card/:cardID/report-virtual-fraud",
    getRoute: (cardID: string) =>
      `settings/card/${cardID}/report-virtual-fraud` as const,
  },
  ...
  SETTINGS_ADD_BANK_ACCOUNT: {
    ...
    getRoute: (backTo?: string, subPage?: string, action?: "edit") => {
      ...
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `settings/wallet/add-bank-account/${subPage}${action ? `/${action}` : ""}`,
        backTo,
      );
    },
  },
  SETTINGS_ADD_US_BANK_ACCOUNT: "settings/wallet/add-us-bank-account",
  SETTINGS_ADD_US_BANK_ACCOUNT_ENTRY_POINT:
    "settings/wallet/add-us-bank-account/entry-point",
  ...
  SETTINGS_WALLET_UNSHARE_BANK_ACCOUNT: {
    ...
    getRoute: (bankAccountID: number | undefined) =>
      `settings/wallet/${bankAccountID}/unshare-bank-account` as const,
  },
  SETTINGS_WALLET_ENABLE_GLOBAL_REIMBURSEMENTS_BUSINESS: {
    route:
      "settings/wallet/:bankAccountID/enable-global-reimbursements/business/:subPage/:action?",
    getRoute: (
  ...
  SETTINGS_WALLET_ENABLE_GLOBAL_REIMBURSEMENTS_AGREEMENTS: {
    route:
      "settings/wallet/:bankAccountID/enable-global-reimbursements/agreements",
    getRoute: (bankAccountID: number | undefined) =>
      `settings/wallet/${bankAccountID}/enable-global-reimbursements/agreements` as const,
  },
  SETTINGS_WALLET_ENABLE_GLOBAL_REIMBURSEMENTS_SIGN: {
    route: "settings/wallet/:bankAccountID/enable-global-reimbursements/sign",
    getRoute: (bankAccountID: number | undefined) =>
      `settings/wallet/${bankAccountID}/enable-global-reimbursements/sign` as const,
  },
  SETTINGS_WALLET_SHARE_BANK_ACCOUNT: {
    route: "settings/wallet/:bankAccountID/share-bank-account",
    getRoute: (bankAccountID: number | undefined) =>
      `settings/wallet/${bankAccountID}/share-bank-account` as const,
  },
  ...
  SETTINGS_WALLET_PERSONAL_CARD_BANK_CONNECTION: {
    ...
    getRoute: (feed: PersonalCardFeed) =>
      `settings/wallet/add-personal-card/${feed}/bank-connection` as const,
  },
  SETTINGS_WALLET_PERSONAL_CARD_UPGRADE:
    "settings/wallet/add-personal-card/upgrade",
  SETTINGS_WALLET_PERSONAL_CARD_WARNING:
    "settings/wallet/add-personal-card/warning",
  ...
  SETTINGS_WALLET_IMPORT_TRANSACTIONS_CARD_NAME:
    "settings/wallet/import-transactions/card-name",
  SETTINGS_WALLET_IMPORT_TRANSACTIONS_CURRENCY:
    "settings/wallet/import-transactions/currency",
  ...
  SETTINGS_WALLET_REPORT_CARD_LOST_OR_DAMAGED_CONFIRM_MAGIC_CODE: {
    ...
    getRoute: (cardID: string, reason: ReplacementReason) =>
      `settings/wallet/card/${cardID}/report-card-lost-or-damaged/${reason}/confirm-magic-code` as const,
  },
  SETTINGS_WALLET_CARD_CHANGE_PIN: {
    route: "settings/wallet/card/:cardID/change-pin",
    getRoute: (cardID: string) =>
      `settings/wallet/card/${cardID}/change-pin` as const,
  },
  SETTINGS_WALLET_CARD_CHANGE_PIN_ATM: {
    route: "settings/wallet/card/:cardID/change-pin-atm",
    getRoute: (cardID: string) =>
      `settings/wallet/card/${cardID}/change-pin-atm` as const,
  },
  ...
  SETTINGS_CONTACT_METHOD_DETAILS: {
    ...
    getRoute: (
      ...
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `settings/profile/contact-methods/${encodedMethod}/details${shouldSkipInitialValidation ? `?shouldSkipInitialValidation=true` : ``}`,
        backTo,
      );
    },
  },
  ...
  SETTINGS_CONTACT_METHOD_VERIFY_ACCOUNT: {
    ...
    getRoute: (backTo?: string, forwardTo?: string) =>
      ...
      getUrlWithBackToParam(
        forwardTo
          ? `settings/profile/contact-methods/verify?forwardTo=${encodeURIComponent(forwardTo)}`
  ...
  SETTINGS_2FA_VERIFY_ACCOUNT: {
    ...
    getRoute: (params: { backTo?: string; forwardTo?: string } = {}) =>
      getUrlWithParams(
        `settings/security/two-factor-auth/${VERIFY_ACCOUNT}`,
        params,
      ),
  },
  SETTINGS_2FA_ROOT: {
  ...
  SETTINGS_2FA_VERIFY: {
    ...
    getRoute: (backTo?: string, forwardTo?: string) =>
      ...
      getUrlWithBackToParam(
        forwardTo
          ? `settings/security/two-factor-auth/verify?forwardTo=${encodeURIComponent(forwardTo)}`
  ...
  SETTINGS_2FA_SUCCESS: {
    ...
    getRoute: (backTo?: string, forwardTo?: string) =>
      ...
      getUrlWithBackToParam(
        forwardTo
          ? `settings/security/two-factor-auth/success?forwardTo=${encodeURIComponent(forwardTo)}`
  ...
  SETTINGS_2FA_REPLACE_VERIFY_OLD:
    "settings/security/two-factor-auth/replace/verify-old",
  SETTINGS_2FA_REPLACE_VERIFY_NEW:
    "settings/security/two-factor-auth/replace/verify-new",
  ...
  NEW_REPORT_WORKSPACE_SELECTION: {
    ...
    getRoute: (isMovingExpenses?: boolean, backTo?: string) => {
      const baseRoute =
        `new-report-workspace-selection${isMovingExpenses ? "?isMovingExpenses=true" : ""}` as const;
  ...
  REPORT_WITH_ID: {
    ...
    getRoute: (
      ...
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `${baseRoute}${queryString}` as const,
        backTo,
      );
    },
  },
  REPORT_CARD_ACTIVATE: {
    ...
    getRoute: (cardID: number, reportID?: string, reportActionID?: string) => {
      if (!reportID) {
        Log.warn(
          "Invalid reportID is used to build the REPORT_CARD_ACTIVATE route",
        );
      }
      if (!reportActionID) {
  ...
  REPORT_ATTACHMENTS: {
    route: "attachment",
    getRoute: (params?: ReportAttachmentsRouteParams) =>
      getAttachmentModalScreenRoute("attachment", params),
  },
  REPORT_ADD_ATTACHMENT: {
    route: "r/:reportID/attachment/add",
    getRoute: (reportID: string, params?: ReportAddAttachmentRouteParams) => {
      return getAttachmentModalScreenRoute(
        `r/${reportID}/attachment/add`,
        params,
      );
    },
  },
  ...
  EDIT_REPORT_FIELD_REQUEST: {
    ...
    getRoute: (
      ...
      if (!policyID || !reportID) {
        Log.warn(
          "Invalid policyID or reportID is used to build the EDIT_REPORT_FIELD_REQUEST route",
          {
            policyID,
            reportID,
          },
      ...
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `r/${reportID}/edit/policyField/${policyID}/${encodeURIComponent(fieldID)}` as const,
        backTo,
      );
    },
  },
  REPORT_WITH_ID_DETAILS_SHARE_CODE: {
    ...
    getRoute: (reportID: string | undefined, backTo?: string) => {
      if (!reportID) {
        Log.warn(
          "Invalid reportID is used to build the REPORT_WITH_ID_DETAILS_SHARE_CODE route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `r/${reportID}/details/shareCode` as const,
        backTo,
      );
    },
  },
  ...
  REPORT_PARTICIPANTS: {
    ...
    getRoute: (reportID: string, backTo?: string) =>
      getUrlWithBackToParam(`r/${reportID}/participants` as const, backTo),
  },
  ...
  REPORT_PARTICIPANTS_DETAILS: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (reportID: string, accountID: number, backTo?: string) =>
      getUrlWithBackToParam(
  ...
  REPORT_WITH_ID_DETAILS: {
    ...
    getRoute: (reportID: string | number | undefined, backTo?: string) => {
      if (!reportID) {
        Log.warn(
          "Invalid reportID is used to build the REPORT_WITH_ID_DETAILS route",
        );
      }
  ...
  REPORT_WITH_ID_DETAILS_EXPORT: {
    ...
    getRoute: (
      ...
      getUrlWithBackToParam(
        `r/${reportID}/details/export/${connectionName as string}` as const,
        backTo,
      ),
  },
  REPORT_WITH_ID_CHANGE_WORKSPACE: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (reportID: string, backTo?: string) =>
      getUrlWithBackToParam(`r/${reportID}/change-workspace` as const, backTo),
  },
  REPORT_SETTINGS: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (reportID: string, backTo?: string) =>
      getUrlWithBackToParam(`r/${reportID}/settings` as const, backTo),
  },
  REPORT_SETTINGS_NAME: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (reportID: string, backTo?: string) =>
      getUrlWithBackToParam(`r/${reportID}/settings/name` as const, backTo),
  },
  REPORT_CHANGE_APPROVER: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (reportID: string, backTo?: string) =>
      getUrlWithBackToParam(`r/${reportID}/change-approver` as const, backTo),
  },
  REPORT_CHANGE_APPROVER_ADD_APPROVER: {
    route: "r/:reportID/change-approver/add",
    getRoute: (reportID: string) =>
      `r/${reportID}/change-approver/add` as const,
  },
  REPORT_SETTINGS_REPORT_LAYOUT: {
    route: "r/:reportID/settings/report-layout",
    getRoute: (reportID: string) =>
      `r/${reportID}/settings/report-layout` as const,
  },
  ...
  SPLIT_BILL_DETAILS: {
    ...
    getRoute: (
      ...
      if (!reportID) {
        Log.warn(
          "Invalid reportID is used to build the SPLIT_BILL_DETAILS route",
        );
      }
  ...
  REPORT_DESCRIPTION: {
    ...
    getRoute: (reportID: string | undefined, backTo?: string) => {
      if (!reportID) {
        Log.warn(
          "Invalid reportID is used to build the REPORT_DESCRIPTION route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `r/${reportID}/description` as const,
        backTo,
      );
    },
  },
  ...
  PRIVATE_NOTES_LIST: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (reportID: string, backTo?: string) =>
      getUrlWithBackToParam(`r/${reportID}/notes` as const, backTo),
  },
  PRIVATE_NOTES_EDIT: {
  ...
  ROOM_MEMBERS: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (reportID: string, backTo?: string) =>
      getUrlWithBackToParam(`r/${reportID}/members` as const, backTo),
  },
  ROOM_MEMBER_DETAILS: {
    ...
    getRoute: (reportID: string, accountID: number, backTo?: string) =>
      getUrlWithBackToParam(
        `r/${reportID}/members/${accountID}` as const,
        backTo,
      ),
  },
  ROOM_INVITE: {
  ...
  SPLIT_EXPENSE: {
    ...
    getRoute: (
      ...
      if (!reportID || !originalTransactionID) {
        Log.warn(
          `Invalid ${reportID}(reportID) or ${originalTransactionID}(transactionID) is used to build the SPLIT_EXPENSE route`,
        );
      }

      const splitExpenseTransactionIDPart = splitExpenseTransactionID
        ? `/${splitExpenseTransactionID}`
        : "/0";

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `create/split-expense/overview/${reportID}/${originalTransactionID}${splitExpenseTransactionIDPart}`,
        backTo,
      );
    },
  },
  SPLIT_EXPENSE_SEARCH: {
    ...
    getRoute: (
      ...
      if (!reportID || !originalTransactionID) {
        Log.warn(
          `Invalid ${reportID}(reportID) or ${originalTransactionID}(transactionID) is used to build the SPLIT_EXPENSE_SEARCH route`,
        );
      }
      ...
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `create/split-expense/overview/${reportID}/${originalTransactionID}${splitExpenseTransactionIDPart}/search`,
        backTo,
      );
    },
  },
  SPLIT_EXPENSE_CREATE_DATE_RANGE: {
    ...
    getRoute: (
      ...
      if (!reportID || !transactionID) {
        Log.warn(
          `Invalid ${reportID}(reportID) or ${transactionID}(transactionID) is used to build the SPLIT_EXPENSE_CREATE_DATE_RANGE route`,
        );
      }
  ...
  SPLIT_EXPENSE_EDIT: {
    route:
      "edit/split-expense/overview/:reportID/:transactionID/:splitExpenseTransactionID?",
    getRoute: (
      reportID: string | undefined,
      originalTransactionID: string | undefined,
      splitExpenseTransactionID?: string,
      ...
      if (!reportID || !originalTransactionID) {
        Log.warn(
          `Invalid ${reportID}(reportID) or ${originalTransactionID}(transactionID) is used to build the SPLIT_EXPENSE_EDIT route`,
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `edit/split-expense/overview/${reportID}/${originalTransactionID}${splitExpenseTransactionID ? `/${splitExpenseTransactionID}` : ""}`,
        backTo,
      );
    },
  },
  ...
  MONEY_REQUEST_STEP_CONFIRMATION: {
    route:
      ":action/:iouType/confirmation/:transactionID/:reportID/:backToReport?",
    getRoute: (
      action: IOUAction,
      iouType: IOUType,
      transactionID: string,
      ...
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `${action as string}/${iouType as string}/confirmation/${transactionID}/${reportID}${optionalRoutePart}` as const,
        backTo,
      );
    },
  },
  ...
  MONEY_REQUEST_STEP_AMOUNT: {
    route:
      ":action/:iouType/amount/:transactionID/:reportID/:reportActionID?/:pageIndex?/:backToReport?",
    getRoute: (
      action: IOUAction,
      iouType: IOUType,
      transactionID: string | undefined,
      ...
      if (!transactionID || !reportID) {
        Log.warn(
          "Invalid transactionID or reportID is used to build the MONEY_REQUEST_STEP_AMOUNT route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `${action as string}/${iouType as string}/amount/${transactionID}/${reportID}/${reportActionID ? `${reportActionID}/` : ""}${pageIndex}`,
        backTo,
      );
    },
  },
  MONEY_REQUEST_STEP_TAX_RATE: {
    ...
    getRoute: (
      ...
      if (!transactionID || !reportID) {
        Log.warn(
          "Invalid transactionID or reportID is used to build the MONEY_REQUEST_STEP_TAX_RATE route",
        );
      }
  ...
  MONEY_REQUEST_STEP_TAX_AMOUNT: {
    ...
    getRoute: (
      ...
      if (!transactionID || !reportID) {
        Log.warn(
          "Invalid transactionID or reportID is used to build the MONEY_REQUEST_STEP_TAX_AMOUNT route",
        );
      }
  ...
  MONEY_REQUEST_STEP_CATEGORY: {
    ...
    getRoute: (
      ...
      if (!transactionID || !reportID) {
        Log.warn(
          "Invalid transactionID or reportID is used to build the MONEY_REQUEST_STEP_CATEGORY route",
        );
      }
  ...
  MONEY_REQUEST_ATTENDEE: {
    ...
    getRoute: (
      ...
      if (!transactionID || !reportID) {
        Log.warn(
          "Invalid transactionID or reportID is used to build the MONEY_REQUEST_ATTENDEE route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `${action as string}/${iouType as string}/attendees/${transactionID}/${reportID}`,
        backTo,
      );
    },
  },
  MONEY_REQUEST_ACCOUNTANT: {
    ...
    getRoute: (
      ...
      if (!transactionID || !reportID) {
        Log.warn(
          "Invalid transactionID or reportID is used to build the MONEY_REQUEST_ACCOUNTANT route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `${action as string}/${iouType as string}/accountant/${transactionID}/${reportID}`,
        backTo,
      );
    },
  },
  MONEY_REQUEST_UPGRADE: {
    ...
    getRoute: (params: {
      ...
      const baseURL =
        `${action as string}/${iouType as string}/upgrade/${transactionID}/${reportID}${upgradePathParam}` as const;

      if (shouldSubmitExpense) {
        // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
        return getUrlWithBackToParam(
          `${baseURL}?shouldSubmitExpense=${shouldSubmitExpense}` as const,
          backTo,
        );
      }
  ...
  MONEY_REQUEST_STEP_DESTINATION: {
    ...
    getRoute: (
      ...
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      getUrlWithBackToParam(
        `${action as string}/${iouType as string}/destination/${transactionID}/${reportID}`,
        backTo,
      ),
  },
  MONEY_REQUEST_STEP_TIME: {
    ...
    getRoute: (
      ...
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      getUrlWithBackToParam(
        `${action as string}/${iouType as string}/time/${transactionID}/${reportID}`,
        backTo,
      ),
  },
  MONEY_REQUEST_STEP_SUBRATE: {
    ...
    getRoute: (
      ...
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      getUrlWithBackToParam(
        `${action as string}/${iouType as string}/subrate/${transactionID}/${reportID}/0`,
        backTo,
      ),
  },
  MONEY_REQUEST_STEP_DESTINATION_EDIT: {
    ...
    getRoute: (
      ...
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      getUrlWithBackToParam(
        `${action as string}/${iouType as string}/destination/${transactionID}/${reportID}/edit`,
        backTo,
      ),
  },
  MONEY_REQUEST_STEP_TIME_EDIT: {
    ...
    getRoute: (
      ...
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      getUrlWithBackToParam(
        `${action as string}/${iouType as string}/time/${transactionID}/${reportID}/edit`,
        backTo,
      ),
  },
  MONEY_REQUEST_STEP_SUBRATE_EDIT: {
    ...
    getRoute: (
      ...
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      getUrlWithBackToParam(
        `${action as string}/${iouType as string}/subrate/${transactionID}/${reportID}/edit/${pageIndex}`,
        backTo,
      ),
  },
  MONEY_REQUEST_STEP_REPORT: {
    ...
    getRoute: (
      ...
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      getUrlWithBackToParam(
        `${action as string}/${iouType as string}/report/${transactionID}/${reportID}${reportActionID ? `/${reportActionID}` : ""}`,
        backTo,
      ),
  },
  MONEY_REQUEST_RECEIPT_PREVIEW: {
    ...
    getRoute: (
      ...
      if (!reportID) {
        Log.warn(
          "Invalid reportID is used to build the MONEY_REQUEST_RECEIPT_PREVIEW route",
        );
      }
      if (!transactionID) {
        Log.warn(
          "Invalid transactionID is used to build the MONEY_REQUEST_RECEIPT_PREVIEW route",
        );
      }
  ...
  MONEY_REQUEST_ODOMETER_PREVIEW: {
    route:
      ":action/:iouType/odometer-image-preview/:transactionID/:reportID/:backToReport?",
    getRoute: (
      reportID: string,
      transactionID: string,
      action: IOUAction,
      ...
      if (!reportID) {
        Log.warn(
          "Invalid reportID is used to build the MONEY_REQUEST_ODOMETER_PREVIEW route",
        );
      }
      if (!transactionID) {
        Log.warn(
          "Invalid transactionID is used to build the MONEY_REQUEST_ODOMETER_PREVIEW route",
        );
      }
  ...
  MONEY_REQUEST_EDIT_REPORT: {
    ...
    getRoute: (
      ...
      if (!reportID) {
        Log.warn(
          "Invalid reportID while building route MONEY_REQUEST_EDIT_REPORT",
        );
      }
      const queryParams = [
        shouldTurnOffSelectionMode ? "shouldTurnOffSelectionMode=true" : "",
      ...
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `${action as string}/${iouType as string}/report/${reportID}/edit${queryString}`,
        backTo,
      );
    },
  },
  SET_DEFAULT_WORKSPACE: {
    ...
    getRoute: (navigateTo?: string) =>
      navigateTo
        ? (`set-default-workspace?navigateTo=${encodeURIComponent(navigateTo)}` as const)
        : ("set-default-workspace" as const),
  },
  SETTINGS_TAGS_ROOT: {
  ...
  SETTINGS_TAGS_SETTINGS: {
    ...
    getRoute: (policyID: string, backTo = "") =>
      getUrlWithBackToParam(
        `settings/${policyID}/tags/settings` as const,
        backTo,
      ),
  },
  SETTINGS_TAGS_EDIT: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (policyID: string, orderWeight: number, backTo = "") =>
      getUrlWithBackToParam(
        `settings/${policyID}/tags/${orderWeight}/edit` as const,
        backTo,
      ),
  },
  SETTINGS_TAG_CREATE: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (policyID: string, backTo = "") =>
      getUrlWithBackToParam(`settings/${policyID}/tags/new` as const, backTo),
  },
  SETTINGS_TAG_EDIT: {
    ...
    getRoute: (
      ...
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      getUrlWithBackToParam(
        `settings/${policyID}/tag/${orderWeight}/${encodeURIComponent(tagName)}/edit` as const,
        backTo,
      ),
  },
  SETTINGS_TAG_SETTINGS: {
    ...
    getRoute: (
      ...
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      getUrlWithBackToParam(
        `settings/${policyID}/tag/${orderWeight}/${encodeURIComponent(tagName)}` as const,
        backTo,
      ),
  },
  SETTINGS_TAG_APPROVER: {
    ...
    getRoute: (
      ...
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      getUrlWithBackToParam(
        `settings/${policyID}/tag/${orderWeight}/${encodeURIComponent(tagName)}/approver` as const,
        backTo,
      ),
  },
  SETTINGS_TAG_LIST_VIEW: {
    ...
    getRoute: (policyID: string, orderWeight: number, backTo = "") =>
      getUrlWithBackToParam(
        `settings/${policyID}/tag-list/${orderWeight}` as const,
        backTo,
      ),
  },
  SETTINGS_TAG_GL_CODE: {
    ...
    getRoute: (
      ...
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      getUrlWithBackToParam(
        `settings/${policyID}/tag/${orderWeight}/${encodeURIComponent(tagName)}/gl-code` as const,
        backTo,
      ),
  },
  SETTINGS_TAGS_IMPORT: {
    ...
    getRoute: (policyID: string, backTo = "") =>
      getUrlWithBackToParam(
        `settings/${policyID}/tags/import` as const,
        backTo,
      ),
  },
  SETTINGS_TAGS_IMPORTED: {
    ...
    getRoute: (policyID: string, backTo = "") =>
      getUrlWithBackToParam(
        `settings/${policyID}/tags/imported` as const,
        backTo,
      ),
  },
  ...
  SETTINGS_CATEGORY_SETTINGS: {
    ...
    getRoute: (policyID: string, categoryName: string, backTo = "") =>
      getUrlWithBackToParam(
        `settings/${policyID}/category/${encodeURIComponent(categoryName)}`,
        backTo,
      ),
  },
  ...
  SETTINGS_CATEGORY_EDIT: {
    ...
    getRoute: (policyID: string, categoryName: string, backTo = "") =>
      getUrlWithBackToParam(
        `settings/${policyID}/category/${encodeURIComponent(categoryName)}/edit`,
        backTo,
      ),
  },
  SETTINGS_CATEGORIES_IMPORT: {
    ...
    getRoute: (policyID: string, backTo = "") =>
      getUrlWithBackToParam(
        `settings/${policyID}/categories/import` as const,
        backTo,
      ),
  },
  SETTINGS_CATEGORIES_IMPORTED: {
    ...
    getRoute: (policyID: string, backTo = "") =>
      getUrlWithBackToParam(
        `settings/${policyID}/categories/imported` as const,
        backTo,
      ),
  },
  SETTINGS_CATEGORY_PAYROLL_CODE: {
    ...
    getRoute: (policyID: string, categoryName: string, backTo = "") =>
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      getUrlWithBackToParam(
        `settings/${policyID}/category/${encodeURIComponent(categoryName)}/payroll-code` as const,
        backTo,
      ),
  },
  SETTINGS_CATEGORY_GL_CODE: {
    ...
    getRoute: (policyID: string, categoryName: string, backTo = "") =>
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      getUrlWithBackToParam(
        `settings/${policyID}/category/${encodeURIComponent(categoryName)}/gl-code` as const,
        backTo,
      ),
  },
  MONEY_REQUEST_STEP_DATE: {
    ...
    getRoute: (
      ...
      if (!transactionID || !reportID) {
        Log.warn(
          "Invalid transactionID or reportID is used to build the MONEY_REQUEST_STEP_DATE route",
        );
      }
  ...
  MONEY_REQUEST_STEP_DESCRIPTION: {
    ...
    getRoute: (
      ...
      if (!transactionID || !reportID) {
        Log.warn(
          "Invalid transactionID or reportID is used to build the MONEY_REQUEST_STEP_DESCRIPTION route",
        );
      }
  ...
  MONEY_REQUEST_STEP_DISTANCE: {
    route:
      ":action/:iouType/distance/:transactionID/:reportID/:reportActionID?",
    getRoute: (
      action: IOUAction,
      iouType: IOUType,
      transactionID: string | undefined,
      ...
      if (!transactionID || !reportID) {
        Log.warn(
          "Invalid transactionID or reportID is used to build the MONEY_REQUEST_STEP_DISTANCE route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `${action as string}/${iouType as string}/distance/${transactionID}/${reportID}${reportActionID ? `/${reportActionID}` : ""}`,
        backTo,
      );
    },
  },
  MONEY_REQUEST_STEP_DISTANCE_MANUAL: {
    route:
      ":action/:iouType/distance-manual/:transactionID/:reportID/:reportActionID?",
    getRoute: (
      action: IOUAction,
      iouType: IOUType,
      transactionID: string | undefined,
      ...
      if (!transactionID || !reportID) {
        Log.warn(
          "Invalid transactionID or reportID is used to build the MONEY_REQUEST_STEP_DISTANCE_MANUAL route",
        );
      }
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `${action as string}/${iouType as string}/distance-manual/${transactionID}/${reportID}${reportActionID ? `/${reportActionID}` : ""}`,
        backTo,
      );
    },
  },
  MONEY_REQUEST_STEP_DISTANCE_ODOMETER: {
    ...
    getRoute: (
      ...
      if (!transactionID || !reportID) {
        Log.warn(
          "Invalid transactionID or reportID is used to build the MONEY_REQUEST_STEP_DISTANCE_ODOMETER route",
        );
      }
  ...
  MONEY_REQUEST_STEP_DISTANCE_RATE: {
    route:
      ":action/:iouType/distanceRate/:transactionID/:reportID/:reportActionID?",
    getRoute: (
      action: IOUAction,
      iouType: IOUType,
      transactionID: string | undefined,
      ...
      if (!transactionID || !reportID) {
        Log.warn(
          "Invalid transactionID or reportID is used to build the MONEY_REQUEST_STEP_DISTANCE_RATE route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `${action as string}/${iouType as string}/distanceRate/${transactionID}/${reportID}${reportActionID ? `/${reportActionID}` : ""}`,
        backTo,
      );
    },
  },
  MONEY_REQUEST_STEP_MERCHANT: {
    route:
      ":action/:iouType/merchant/:transactionID/:reportID/:reportActionID?",
    getRoute: (
      action: IOUAction,
      iouType: IOUType,
      transactionID: string | undefined,
      ...
      if (!transactionID || !reportID) {
        Log.warn(
          "Invalid transactionID or reportID is used to build the MONEY_REQUEST_STEP_MERCHANT route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `${action as string}/${iouType as string}/merchant/${transactionID}/${reportID}${reportActionID ? `/${reportActionID}` : ""}`,
        backTo,
      );
    },
  },
  ...
  MONEY_REQUEST_STEP_SCAN: {
    ...
    getRoute: (
      ...
      if (!transactionID || !reportID) {
        Log.warn(
          "Invalid transactionID or reportID is used to build the MONEY_REQUEST_STEP_SCAN route",
        );
      }
  ...
  MONEY_REQUEST_START: {
    ...
    getRoute: (iouType: IOUType, iouRequestType: IOURequestType) =>
      `start/${iouType as string}/${iouRequestType as string}` as const,
  },
  ...
  MONEY_REQUEST_RECEIPT_VIEW: {
    ...
    getRoute: (transactionID: string, backTo: string) =>
      getUrlWithBackToParam(`receipt-view/${transactionID}`, backTo),
  } as const,
  ...
  MONEY_REQUEST_STEP_HOURS_EDIT: {
    route:
      ":action/:iouType/hours-edit/:transactionID/:reportID/:reportActionID?",
    getRoute: (
      action: IOUAction,
      iouType: IOUType,
      transactionID: string | undefined,
  ...
  DISTANCE_REQUEST_CREATE: {
    route:
      ":action/:iouType/start/:transactionID/:reportID/distance-new/:backToReport?",
    getRoute: (
      action: IOUAction,
      iouType: IOUType,
      transactionID: string,
  ...
  ODOMETER_IMAGE: {
    route:
      ":action/:iouType/odometer-image/:transactionID/:reportID/:imageType/:backToReport?",
    getRoute: (
      action: IOUAction,
      iouType: IOUType,
      transactionID: string,
  ...
  NEW_TASK_ASSIGNEE: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam("new/task/assignee", backTo),
  },
  ...
  NEW_TASK_DETAILS: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam("new/task/details", backTo),
  },
  NEW_TASK_TITLE: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam("new/task/title", backTo),
  },
  NEW_TASK_DESCRIPTION: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam("new/task/description", backTo),
  },
  ...
  WORKSPACE_INITIAL: {
    ...
    getRoute: (policyID: string | undefined, backTo?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_INITIAL route",
        );
      }
  ...
  WORKSPACE_INVITE_MESSAGE_APPROVER: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (policyID: string, backTo?: string) =>
      `${getUrlWithBackToParam(`workspaces/${policyID}/invite-message/approver`, backTo)}` as const,
  },
  WORKSPACE_OVERVIEW: {
    ...
    getRoute: (policyID: string | undefined, backTo?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_OVERVIEW route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `workspaces/${policyID}/overview` as const,
        backTo,
      );
    },
  },
  WORKSPACE_ACCOUNTING: {
    ...
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting` as const,
  },
  ...
  POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_EXPORT: {
    ...
    getRoute: (policyID: string | undefined, backTo?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_EXPORT route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/quickbooks-online/export` as const,
        backTo,
        false,
      );
    },
  ...
  POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_COMPANY_CARD_EXPENSE_ACCOUNT: {
    ...
    getRoute: (policyID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/quickbooks-online/export/company-card-expense-account` as const,
        backTo,
      ),
  },
  POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_COMPANY_CARD_EXPENSE_ACCOUNT_SELECT: {
    ...
    getRoute: (policyID: string | undefined, backTo?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_COMPANY_CARD_EXPENSE_ACCOUNT_SELECT route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/quickbooks-online/export/company-card-expense-account/account-select` as const,
        backTo,
      );
    },
  },
  POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_NON_REIMBURSABLE_DEFAULT_VENDOR_SELECT: {
    route:
      "workspaces/:policyID/accounting/quickbooks-online/export/company-card-expense-account/default-vendor-select",
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_NON_REIMBURSABLE_DEFAULT_VENDOR_SELECT route",
        );
      }
  ...
  POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_COMPANY_CARD_EXPENSE_SELECT: {
    ...
    getRoute: (policyID: string | undefined, backTo?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_COMPANY_CARD_EXPENSE_SELECT route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/quickbooks-online/export/company-card-expense-account/card-select` as const,
        backTo,
      );
    },
  },
  POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_INVOICE_ACCOUNT_SELECT: {
    ...
    getRoute: (policyID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/quickbooks-online/export/invoice-account-select` as const,
        backTo,
      ),
  },
  POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_TRAVEL_INVOICING_CONFIGURATION: {
    route:
      "workspaces/:policyID/accounting/quickbooks-online/export/travel-invoicing",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/quickbooks-online/export/travel-invoicing` as const,
  },
  POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_TRAVEL_INVOICING_VENDOR_SELECT: {
    route:
      "workspaces/:policyID/accounting/quickbooks-online/export/travel-invoicing/vendor",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/quickbooks-online/export/travel-invoicing/vendor` as const,
  },
  POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_TRAVEL_INVOICING_PAYABLE_ACCOUNT_SELECT: {
    route:
      "workspaces/:policyID/accounting/quickbooks-online/export/travel-invoicing/payable-account",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/quickbooks-online/export/travel-invoicing/payable-account` as const,
  },
  POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_COMPANY_CARD_EXPENSE_ACCOUNT_SELECT: {
    route:
      "workspaces/:policyID/accounting/quickbooks-desktop/export/company-card-expense-account/account-select",
    getRoute: (policyID?: string, backTo?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_COMPANY_CARD_EXPENSE_ACCOUNT_SELECT route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/quickbooks-desktop/export/company-card-expense-account/account-select` as const,
        backTo,
      );
    },
  },
  POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_COMPANY_CARD_EXPENSE_SELECT: {
    ...
    getRoute: (policyID?: string, backTo?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_COMPANY_CARD_EXPENSE_SELECT route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/quickbooks-desktop/export/company-card-expense-account/card-select` as const,
        backTo,
      );
    },
  },
  POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_NON_REIMBURSABLE_DEFAULT_VENDOR_SELECT: {
    route:
      "workspaces/:policyID/accounting/quickbooks-desktop/export/company-card-expense-account/default-vendor-select",
    getRoute: (policyID?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_NON_REIMBURSABLE_DEFAULT_VENDOR_SELECT route",
        );
      }
  ...
  POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_COMPANY_CARD_EXPENSE_ACCOUNT: {
    route:
      "workspaces/:policyID/accounting/quickbooks-desktop/export/company-card-expense-account",
    getRoute: (policyID?: string, backTo?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_COMPANY_CARD_EXPENSE_ACCOUNT route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/quickbooks-desktop/export/company-card-expense-account` as const,
        backTo,
      );
    },
  },
  WORKSPACE_ACCOUNTING_QUICKBOOKS_DESKTOP_ADVANCED: {
    ...
    getRoute: (policyID?: string, backTo?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_ACCOUNTING_QUICKBOOKS_DESKTOP_ADVANCED route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/quickbooks-desktop/advanced` as const,
        backTo,
      );
    },
  },
  POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_AUTO_SYNC: {
    route:
      "workspaces/:policyID/accounting/quickbooks-desktop/advanced/autosync",
    getRoute: (policyID?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_AUTO_SYNC route",
        );
      }
  ...
  POLICY_ACCOUNTING_CARD_RECONCILIATION_QUICKBOOKS_DESKTOP_AUTO_SYNC: {
    route:
      "workspaces/:policyID/accounting/quickbooks-desktop/card-reconciliation/autosync",
    getRoute: (policyID?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_CARD_RECONCILIATION_QUICKBOOKS_DESKTOP_AUTO_SYNC route",
        );
      }
  ...
  POLICY_ACCOUNTING_CARD_RECONCILIATION_SAGE_INTACCT_AUTO_SYNC: {
    route:
      "workspaces/:policyID/accounting/sage-intacct/card-reconciliation/autosync",
    getRoute: (policyID?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_CARD_RECONCILIATION_SAGE_INTACCT_AUTO_SYNC route",
        );
      }
  ...
  POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_ACCOUNTING_METHOD: {
    route:
      "workspaces/:policyID/accounting/quickbooks-desktop/advanced/autosync/accounting-method",
    getRoute: (policyID?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_ACCOUNTING_METHOD route",
        );
      }
  ...
  POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_EXPORT_DATE_SELECT: {
    ...
    getRoute: (policyID?: string, backTo?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_EXPORT_DATE_SELECT route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/quickbooks-desktop/export/date-select` as const,
        backTo,
      );
    },
  },
  POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_PREFERRED_EXPORTER: {
    ...
    getRoute: (policyID?: string, backTo?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_PREFERRED_EXPORTER route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/quickbooks-desktop/export/preferred-exporter` as const,
        backTo,
      );
    },
  },
  POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_EXPORT_OUT_OF_POCKET_EXPENSES: {
    ...
    getRoute: (policyID?: string, backTo?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_EXPORT_OUT_OF_POCKET_EXPENSES route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/quickbooks-desktop/export/out-of-pocket-expense` as const,
        backTo,
      );
    },
  },
  POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_EXPORT_OUT_OF_POCKET_EXPENSES_ACCOUNT_SELECT:
    {
      ...
      getRoute: (policyID?: string, backTo?: string) => {
        if (!policyID) {
          Log.warn(
            "Invalid policyID is used to build the POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_EXPORT_OUT_OF_POCKET_EXPENSES_ACCOUNT_SELECT route",
          );
        }

        // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
        return getUrlWithBackToParam(
          `workspaces/${policyID}/accounting/quickbooks-desktop/export/out-of-pocket-expense/account-select` as const,
          backTo,
        );
      },
    },
  POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_EXPORT_OUT_OF_POCKET_EXPENSES_SELECT: {
    ...
    getRoute: (policyID?: string, backTo?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_EXPORT_OUT_OF_POCKET_EXPENSES_SELECT route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/quickbooks-desktop/export/out-of-pocket-expense/entity-select` as const,
        backTo,
      );
    },
  },
  POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_EXPORT: {
    ...
    getRoute: (policyID: string | undefined, backTo?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_EXPORT route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/quickbooks-desktop/export` as const,
        backTo,
        false,
      );
    },
  ...
  POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_SETUP_MODAL: {
    ...
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/quickbooks-desktop/setup-modal` as const,
  },
  POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_SETUP_REQUIRED_DEVICE_MODAL: {
    route:
      "workspaces/:policyID/accounting/quickbooks-desktop/setup-required-device",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/quickbooks-desktop/setup-required-device` as const,
  },
  POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_TRIGGER_FIRST_SYNC: {
    route:
      "workspaces/:policyID/accounting/quickbooks-desktop/trigger-first-sync",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/quickbooks-desktop/trigger-first-sync` as const,
  },
  POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_IMPORT: {
    route: "workspaces/:policyID/accounting/quickbooks-desktop/import",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/quickbooks-desktop/import` as const,
  },
  POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_CHART_OF_ACCOUNTS: {
    ...
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/quickbooks-desktop/import/accounts` as const,
  },
  POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_CLASSES: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_CLASSES route",
        );
      }
  ...
  POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_CLASSES_DISPLAYED_AS: {
    route:
      "workspaces/:policyID/accounting/quickbooks-desktop/import/classes/displayed_as",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/quickbooks-desktop/import/classes/displayed_as` as const,
  },
  POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_CUSTOMERS: {
    route:
      "workspaces/:policyID/accounting/quickbooks-desktop/import/customers",
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_CUSTOMERS route",
        );
      }
  ...
  POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_CUSTOMERS_DISPLAYED_AS: {
    route:
      "workspaces/:policyID/accounting/quickbooks-desktop/import/customers/displayed_as",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/quickbooks-desktop/import/customers/displayed_as` as const,
  },
  POLICY_ACCOUNTING_QUICKBOOKS_DESKTOP_ITEMS: {
    route: "workspaces/:policyID/accounting/quickbooks-desktop/import/items",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/quickbooks-desktop/import/items` as const,
  },
  WORKSPACE_OVERVIEW_NAME: {
    route: "workspaces/:policyID/overview/name",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/overview/name` as const,
  },
  WORKSPACE_OVERVIEW_DESCRIPTION: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_OVERVIEW_DESCRIPTION route",
        );
      }
  ...
  WORKSPACE_OVERVIEW_CLIENT_ID: {
    ...
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/overview/client-id` as const,
  },
  WORKSPACE_OVERVIEW_SHARE: {
    route: "workspaces/:policyID/overview/share",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/overview/share` as const,
  },
  WORKSPACE_AVATAR: {
    route: "workspaces/:policyID/avatar",
    getRoute: (policyID: string, fallbackLetter?: UpperCaseCharacters) =>
      `workspaces/${policyID}/avatar${fallbackLetter ? `?letter=${fallbackLetter}` : ""}` as const,
  },
  WORKSPACE_JOIN_USER: {
    route: "workspaces/:policyID/join",
    getRoute: (policyID: string, inviterEmail: string) =>
      `workspaces/${policyID}/join?email=${inviterEmail}` as const,
  },
  WORKSPACE_SETTINGS_CURRENCY: {
    route: "workspaces/:policyID/settings/currency",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/settings/currency` as const,
  },
  WORKSPACE_WORKFLOWS: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_WORKFLOWS route",
        );
      }
  ...
  WORKSPACE_WORKFLOWS_APPROVALS_NEW: {
    ...
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/workflows/approvals/new` as const,
  },
  WORKSPACE_WORKFLOWS_APPROVALS_EDIT: {
    ...
    getRoute: (policyID: string, firstApproverEmail: string) =>
      `workspaces/${policyID}/workflows/approvals/${encodeURIComponent(firstApproverEmail)}/edit` as const,
  },
  WORKSPACE_WORKFLOWS_APPROVALS_EXPENSES_FROM: {
    ...
    getRoute: (policyID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/workflows/approvals/expenses-from` as const,
        backTo,
      ),
  },
  WORKSPACE_WORKFLOWS_APPROVALS_APPROVER: {
    ...
    getRoute: (policyID: string, approverIndex: number) =>
      `workspaces/${policyID}/workflows/approvals/approver?approverIndex=${approverIndex}` as const,
  },
  WORKSPACE_WORKFLOWS_APPROVALS_APPROVER_CHANGE: {
    route: "workspaces/:policyID/workflows/approvals/approver-change",
    getRoute: (policyID: string, approverIndex: number) =>
      `workspaces/${policyID}/workflows/approvals/approver-change?approverIndex=${approverIndex}` as const,
  },
  WORKSPACE_WORKFLOWS_APPROVALS_APPROVAL_LIMIT: {
    route: "workspaces/:policyID/workflows/approvals/approval-limit",
    getRoute: (policyID: string, approverIndex: number) =>
      `workspaces/${policyID}/workflows/approvals/approval-limit?approverIndex=${approverIndex}` as const,
  },
  WORKSPACE_WORKFLOWS_APPROVALS_OVER_LIMIT_APPROVER: {
    route: "workspaces/:policyID/workflows/approvals/over-limit-approver",
    getRoute: (policyID: string, approverIndex: number) =>
      `workspaces/${policyID}/workflows/approvals/over-limit-approver?approverIndex=${approverIndex}` as const,
  },
  WORKSPACE_WORKFLOWS_PAYER: {
    route: "workspaces/:policyID/workflows/payer",
    getRoute: (policyId: string) =>
      `workspaces/${policyId}/workflows/payer` as const,
  },
  WORKSPACE_WORKFLOWS_AUTOREPORTING_FREQUENCY: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_WORKFLOWS_AUTOREPORTING_FREQUENCY route",
        );
      }
  ...
  WORKSPACE_WORKFLOWS_AUTOREPORTING_MONTHLY_OFFSET: {
    route:
      "workspaces/:policyID/workflows/auto-reporting-frequency/monthly-offset",
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_WORKFLOWS_AUTOREPORTING_MONTHLY_OFFSET route",
        );
      }
  ...
  WORKSPACE_INVOICES: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_INVOICES route",
        );
      }
  ...
  WORKSPACE_MEMBERS: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_MEMBERS route",
        );
      }
  ...
  POLICY_ACCOUNTING: {
    ...
    getRoute: (
      ...
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING route",
        );
      }
  ...
  WORKSPACE_ACCOUNTING_QUICKBOOKS_ONLINE_ADVANCED: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_ACCOUNTING_QUICKBOOKS_ONLINE_ADVANCED route",
        );
      }
  ...
  WORKSPACE_ACCOUNTING_QUICKBOOKS_ONLINE_ACCOUNT_SELECTOR: {
    ...
    getRoute: (policyID: string | undefined) =>
      `workspaces/${policyID}/accounting/quickbooks-online/account-selector` as const,
  },
  WORKSPACE_ACCOUNTING_QUICKBOOKS_ONLINE_INVOICE_ACCOUNT_SELECTOR: {
    route:
      "workspaces/:policyID/accounting/quickbooks-online/invoice-account-selector",
    getRoute: (policyID: string | undefined) =>
      `workspaces/${policyID}/accounting/quickbooks-online/invoice-account-selector` as const,
  },
  WORKSPACE_ACCOUNTING_QUICKBOOKS_ONLINE_AUTO_SYNC: {
    route:
      "workspaces/:policyID/connections/quickbooks-online/advanced/autosync",

    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (policyID: string | undefined, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/connections/quickbooks-online/advanced/autosync` as const,
        backTo,
      ),
  },
  WORKSPACE_ACCOUNTING_QUICKBOOKS_ONLINE_ACCOUNTING_METHOD: {
    route:
      "workspaces/:policyID/connections/quickbooks-online/advanced/autosync/accounting-method",
    getRoute: (policyID: string | undefined, backTo?: string) =>
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      getUrlWithBackToParam(
        `workspaces/${policyID}/connections/quickbooks-online/advanced/autosync/accounting-method` as const,
        backTo,
      ),
  },
  WORKSPACE_ACCOUNTING_CARD_RECONCILIATION: {
    ...
    getRoute: (
      ...
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_ACCOUNTING_CARD_RECONCILIATION route",
        );
      }
  ...
  WORKSPACE_ACCOUNTING_RECONCILIATION_ACCOUNT_SETTINGS: {
    ...
    getRoute: (
      ...
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_ACCOUNTING_RECONCILIATION_ACCOUNT_SETTINGS route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/${connection as string}/card-reconciliation/account` as const,
        backTo,
      );
    },
  },
  WORKSPACE_CATEGORIES: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_CATEGORIES route",
        );
      }
  ...
  WORKSPACE_CATEGORY_SETTINGS: {
    ...
    getRoute: (policyID: string, categoryName: string) =>
      `workspaces/${policyID}/category/${encodeURIComponent(categoryName)}` as const,
  },
  WORKSPACE_UPGRADE: {
    ...
    getRoute: (policyID?: string, featureName?: string, backTo?: string) =>
      ...
      getUrlWithBackToParam(
        policyID
          ? (`workspaces/${policyID}/upgrade/${encodeURIComponent(featureName ?? "")}` as const)
          : (`workspaces/upgrade` as const),
        backTo,
      ),
  },
  WORKSPACE_DOWNGRADE: {
    ...
    getRoute: (policyID?: string, backTo?: string) =>
      getUrlWithBackToParam(
        policyID
          ? (`workspaces/${policyID}/downgrade/` as const)
          : (`workspaces/downgrade` as const),
        backTo,
      ),
  },
  WORKSPACE_PAY_AND_DOWNGRADE: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam(`workspaces/pay-and-downgrade` as const, backTo),
  },
  WORKSPACE_CATEGORIES_SETTINGS: {
    route: "workspaces/:policyID/categories/settings",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/categories/settings` as const,
  },
  WORKSPACE_CATEGORIES_IMPORT: {
    route: "workspaces/:policyID/categories/import",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/categories/import` as const,
  },
  WORKSPACE_CATEGORIES_IMPORTED: {
    route: "workspaces/:policyID/categories/imported",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/categories/imported` as const,
  },
  WORKSPACE_CATEGORY_CREATE: {
    route: "workspaces/:policyID/categories/new",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/categories/new` as const,
  },
  WORKSPACE_CATEGORY_EDIT: {
    route: "workspaces/:policyID/category/:categoryName/edit",
    getRoute: (policyID: string, categoryName: string) =>
      `workspaces/${policyID}/category/${encodeURIComponent(categoryName)}/edit` as const,
  },
  WORKSPACE_CATEGORY_PAYROLL_CODE: {
    route: "workspaces/:policyID/category/:categoryName/payroll-code",
    getRoute: (policyID: string, categoryName: string) =>
      `workspaces/${policyID}/category/${encodeURIComponent(categoryName)}/payroll-code` as const,
  },
  WORKSPACE_CATEGORY_GL_CODE: {
    route: "workspaces/:policyID/category/:categoryName/gl-code",
    getRoute: (policyID: string, categoryName: string) =>
      `workspaces/${policyID}/category/${encodeURIComponent(categoryName)}/gl-code` as const,
  },
  WORKSPACE_CATEGORY_DEFAULT_TAX_RATE: {
    route: "workspaces/:policyID/category/:categoryName/tax-rate",
    getRoute: (policyID: string, categoryName: string) =>
      `workspaces/${policyID}/category/${encodeURIComponent(categoryName)}/tax-rate` as const,
  },
  WORKSPACE_CATEGORY_FLAG_AMOUNTS_OVER: {
    route: "workspaces/:policyID/category/:categoryName/flag-amounts",
    getRoute: (policyID: string, categoryName: string) =>
      `workspaces/${policyID}/category/${encodeURIComponent(categoryName)}/flag-amounts` as const,
  },
  WORKSPACE_CATEGORY_DESCRIPTION_HINT: {
    route: "workspaces/:policyID/category/:categoryName/description-hint",
    getRoute: (policyID: string, categoryName: string) =>
      `workspaces/${policyID}/category/${encodeURIComponent(categoryName)}/description-hint` as const,
  },
  WORKSPACE_CATEGORY_REQUIRE_RECEIPTS_OVER: {
    route: "workspaces/:policyID/category/:categoryName/require-receipts-over",
    getRoute: (policyID: string, categoryName: string) =>
      `workspaces/${policyID}/category/${encodeURIComponent(categoryName)}/require-receipts-over` as const,
  },
  WORKSPACE_CATEGORY_REQUIRE_ITEMIZED_RECEIPTS_OVER: {
    route:
      "workspaces/:policyID/category/:categoryName/require-itemized-receipts-over",
    getRoute: (policyID: string, categoryName: string) =>
      `workspaces/${policyID}/category/${encodeURIComponent(categoryName)}/require-itemized-receipts-over` as const,
  },
  WORKSPACE_CATEGORY_REQUIRED_FIELDS: {
    route: "workspaces/:policyID/category/:categoryName/required-fields",
    getRoute: (policyID: string, categoryName: string) =>
      `workspaces/${policyID}/category/${encodeURIComponent(categoryName)}/required-fields` as const,
  },
  WORKSPACE_CATEGORY_APPROVER: {
    route: "workspaces/:policyID/category/:categoryName/approver",
    getRoute: (policyID: string, categoryName: string) =>
      `workspaces/${policyID}/category/${encodeURIComponent(categoryName)}/approver` as const,
  },
  WORKSPACE_MORE_FEATURES: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_MORE_FEATURES route",
        );
      }
  ...
  WORKSPACE_TAGS_SETTINGS: {
    ...
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/tags/settings` as const,
  },
  WORKSPACE_EDIT_TAGS: {
    route: "workspaces/:policyID/tags/:orderWeight/edit",

    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (policyID: string, orderWeight: number, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/tags/${orderWeight}/edit` as const,
        backTo,
      ),
  },
  WORKSPACE_TAG_EDIT: {
    ...
    getRoute: (policyID: string, orderWeight: number, tagName: string) =>
      `workspaces/${policyID}/tag/${orderWeight}/${encodeURIComponent(tagName)}/edit` as const,
  },
  ...
  WORKSPACE_CUSTOM_FIELDS: {
    ...
    getRoute: (
      ...
      `/workspaces/${policyID}/members/${accountID}/${customFieldType}` as const,
  },
  WORKSPACE_MEMBER_NEW_CARD: {
    route: "workspaces/:policyID/members/:accountID/new-card",
    getRoute: (policyID: string, accountID: number) =>
      `workspaces/${policyID}/members/${accountID}/new-card` as const,
  },
  WORKSPACE_MEMBER_ROLE_SELECTION: {
    route: "workspaces/:policyID/members/:accountID/role-selection",
    getRoute: (policyID: string, accountID: number) =>
      `workspaces/${policyID}/members/${accountID}/role-selection` as const,
  },
  WORKSPACE_OWNER_CHANGE_SUCCESS: {
    route: "workspaces/:policyID/change-owner/:accountID/success",

    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (policyID: string, accountID: number, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/change-owner/${accountID}/success` as const,
        backTo,
      ),
  },
  WORKSPACE_OWNER_CHANGE_ERROR: {
    ...
    getRoute: (policyID: string, accountID: number, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/change-owner/${accountID}/failure` as const,
        backTo,
      ),
  },
  WORKSPACE_OWNER_CHANGE_CHECK: {
    ...
    getRoute: (
      ...
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_OWNER_CHANGE_CHECK route",
        );
      }
  ...
  WORKSPACE_TAX_EDIT: {
    ...
    getRoute: (policyID: string, taxID: string) =>
      `workspaces/${policyID}/tax/${encodeURIComponent(taxID)}` as const,
  },
  WORKSPACE_TAX_NAME: {
    route: "workspaces/:policyID/tax/:taxID/name",
    getRoute: (policyID: string, taxID: string) =>
      `workspaces/${policyID}/tax/${encodeURIComponent(taxID)}/name` as const,
  },
  WORKSPACE_TAX_VALUE: {
    route: "workspaces/:policyID/tax/:taxID/value",
    getRoute: (policyID: string, taxID: string) =>
      `workspaces/${policyID}/tax/${encodeURIComponent(taxID)}/value` as const,
  },
  WORKSPACE_TAX_CODE: {
    route: "workspaces/:policyID/tax/:taxID/tax-code",
    getRoute: (policyID: string, taxID: string) =>
      `workspaces/${policyID}/tax/${encodeURIComponent(taxID)}/tax-code` as const,
  },
  WORKSPACE_REPORTS: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_REPORTS route",
        );
      }
  ...
  WORKSPACE_CREATE_REPORT_FIELD: {
    ...
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/reports/newReportField` as const,
  },
  WORKSPACE_REPORT_FIELDS_SETTINGS: {
    ...
    getRoute: (policyID: string, reportFieldID: string) =>
      `workspaces/${policyID}/reports/${encodeURIComponent(reportFieldID)}/edit` as const,
  },
  WORKSPACE_REPORT_FIELDS_LIST_VALUES: {
    route: "workspaces/:policyID/reports/listValues/:reportFieldID?",
    getRoute: (policyID: string, reportFieldID?: string) =>
      `workspaces/${policyID}/reports/listValues/${reportFieldID ? encodeURIComponent(reportFieldID) : ""}` as const,
  },
  WORKSPACE_REPORT_FIELDS_ADD_VALUE: {
    route: "workspaces/:policyID/reports/addValue/:reportFieldID?",
    getRoute: (policyID: string, reportFieldID?: string) =>
      `workspaces/${policyID}/reports/addValue/${reportFieldID ? encodeURIComponent(reportFieldID) : ""}` as const,
  },
  ...
  WORKSPACE_REPORT_FIELDS_EDIT_VALUE: {
    ...
    getRoute: (policyID: string, valueIndex: number) =>
      `workspaces/${policyID}/reports/newReportField/${valueIndex}/edit` as const,
  },
  WORKSPACE_EDIT_REPORT_FIELDS_INITIAL_VALUE: {
    route: "workspaces/:policyID/reports/:reportFieldID/edit/initialValue",
    getRoute: (policyID: string, reportFieldID: string) =>
      `workspaces/${policyID}/reports/${encodeURIComponent(reportFieldID)}/edit/initialValue` as const,
  },
  WORKSPACE_COMPANY_CARDS: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_COMPANY_CARDS route",
        );
      }
  ...
  WORKSPACE_COMPANY_CARDS_BANK_CONNECTION: {
    ...
    getRoute: (
      ...
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_COMPANY_CARDS_BANK_CONNECTION route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `workspaces/${policyID}/company-cards/${feed}/bank-connection`,
        backTo,
      );
    },
  },
  WORKSPACE_COMPANY_CARDS_ADD_NEW: {
    ...
    getRoute: (policyID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/company-cards/add-card-feed`,
        backTo,
      ),
  },
  WORKSPACE_COMPANY_CARDS_IMPORT_SPREADSHEET: {
    ...
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/company-cards/add-card-feed/import` as const,
  },
  WORKSPACE_COMPANY_CARDS_IMPORTED: {
    route: "workspaces/:policyID/company-cards/add-card-feed/import/mapping",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/company-cards/add-card-feed/import/mapping` as const,
  },
  WORKSPACE_COMPANY_CARDS_LAYOUT_NAME: {
    route: "workspaces/:policyID/company-cards/add-card-feed/layout-name",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/company-cards/add-card-feed/layout-name` as const,
  },
  WORKSPACE_COMPANY_CARDS_SELECT_FEED: {
    route: "workspaces/:policyID/company-cards/select-feed",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/company-cards/select-feed` as const,
  },
  WORKSPACE_COMPANY_CARDS_BROKEN_CARD_FEED_CONNECTION: {
    route:
      "workspaces/:policyID/company-cards/:feed/broken-card-feed-connection",
    getRoute: (policyID: string, feed: CompanyCardFeedWithDomainID) =>
      `workspaces/${policyID}/company-cards/${encodeURIComponent(feed)}/broken-card-feed-connection` as const,
  },
  WORKSPACE_COMPANY_CARDS_REFRESH_CARD_FEED_CONNECTION: {
    route:
      "workspaces/:policyID/company-cards/:feed/refresh-card-feed-connection",
    getRoute: (policyID: string, feed: CompanyCardFeedWithDomainID) =>
      `workspaces/${policyID}/company-cards/${encodeURIComponent(feed)}/refresh-card-feed-connection` as const,
  },
  WORKSPACE_COMPANY_CARDS_VERIFY_ACCOUNT: {
    route: `workspaces/:policyID/company-cards/${VERIFY_ACCOUNT}`,
    getRoute: (policyID: string, feed?: CompanyCardFeedWithDomainID) =>
      feed
        ? (`workspaces/${policyID}/company-cards/${VERIFY_ACCOUNT}?feed=${encodeURIComponent(feed)}` as const)
        : (`workspaces/${policyID}/company-cards/${VERIFY_ACCOUNT}` as const),
  },
  WORKSPACE_COMPANY_CARDS_ASSIGN_CARD_ASSIGNEE: {
    route:
      "workspaces/:policyID/company-cards/:feed/assign-card/:cardID/assignee",
    getRoute: (
      params: WorkspaceCompanyCardsAssignCardParams,
      backTo?: string,
    ) =>
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      getUrlWithBackToParam(
  ...
  WORKSPACE_COMPANY_CARDS_ASSIGN_CARD_CARD_SELECTION: {
    route:
      "workspaces/:policyID/company-cards/:feed/assign-card/:cardID/card-selection",
  ...
  WORKSPACE_COMPANY_CARDS_ASSIGN_CARD_TRANSACTION_START_DATE: {
    route:
      "workspaces/:policyID/company-cards/:feed/assign-card/:cardID/transaction-start-date",
    getRoute: (params: WorkspaceCompanyCardsAssignCardParams) =>
  ...
  WORKSPACE_COMPANY_CARDS_ASSIGN_CARD_CARD_NAME: {
    route:
      "workspaces/:policyID/company-cards/:feed/assign-card/:cardID/card-name",
    getRoute: (params: WorkspaceCompanyCardsAssignCardParams) =>
  ...
  WORKSPACE_COMPANY_CARDS_ASSIGN_CARD_CONFIRMATION: {
    route:
      "workspaces/:policyID/company-cards/:feed/assign-card/:cardID/confirmation",
    getRoute: (
      params: WorkspaceCompanyCardsAssignCardParams,
      backTo?: string,
    ) =>
  ...
  WORKSPACE_COMPANY_CARDS_ASSIGN_CARD_INVITE_NEW_MEMBER: {
    route:
      "workspaces/:policyID/company-cards/:feed/assign-card/:cardID/invite-new-member",
    getRoute: (params: WorkspaceCompanyCardsAssignCardParams) =>
  ...
  WORKSPACE_COMPANY_CARD_DETAILS: {
    ...
    getRoute: (
      ...
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      getUrlWithBackToParam(
        `workspaces/${policyID}/company-cards/${encodeURIComponent(feed)}/${encodeURIComponent(cardID)}`,
        backTo,
      ),
  },
  ...
  WORKSPACE_COMPANY_CARD_ADD_WORK_EMAIL: {
    ...
    getRoute: (policyID: string, feed: CompanyCardFeedWithDomainID) =>
      `workspaces/${policyID}/company-cards/${encodeURIComponent(feed)}/work-email` as const,
  },
  WORKSPACE_COMPANY_CARD_VERIFY_WORK_EMAIL: {
    route: "workspaces/:policyID/company-cards/:feed/verify-work-email",
    getRoute: (policyID: string, feed: CompanyCardFeedWithDomainID) =>
      `workspaces/${policyID}/company-cards/${encodeURIComponent(feed)}/verify-work-email` as const,
  },
  WORKSPACE_COMPANY_CARD_EDIT_TRANSACTION_START_DATE: {
    route:
      "workspaces/:policyID/company-cards/:feed/:cardID/edit/transaction-start-date",
    getRoute: (
      policyID: string,
      cardID: string,
      feed: CompanyCardFeedWithDomainID,
  ...
  WORKSPACE_COMPANY_CARD_EXPORT: {
    ...
    getRoute: (
      ...
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      getUrlWithBackToParam(
        `workspaces/${policyID}/company-cards/${feed}/${cardID}/edit/export`,
        backTo,
        false,
      ),
  },
  WORKSPACE_EXPENSIFY_CARD: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_EXPENSIFY_CARD route",
        );
      }
  ...
  WORKSPACE_EXPENSIFY_CARD_DETAILS: {
    ...
    getRoute: (policyID: string, cardID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/expensify-card/${cardID}`,
        backTo,
      ),
  },
  ...
  WORKSPACE_EXPENSIFY_CARD_NAME: {
    ...
    getRoute: (policyID: string, cardID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/expensify-card/${cardID}/edit/name`,
        backTo,
      ),
  },
  EXPENSIFY_CARD_NAME: {
  ...
  WORKSPACE_EXPENSIFY_CARD_LIMIT: {
    ...
    getRoute: (policyID: string, cardID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/expensify-card/${cardID}/edit/limit`,
        backTo,
      ),
  },
  ...
  WORKSPACE_EXPENSIFY_CARD_LIMIT_TYPE: {
    ...
    getRoute: (policyID: string, cardID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/expensify-card/${cardID}/edit/limit-type`,
        backTo,
      ),
  },
  ...
  EXPENSIFY_CARD_EXPIRY_OPTIONS: {
    ...
    getRoute: (policyID: string, cardID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `settings/${policyID}/expensify-card/${cardID}/edit/expiry-options`,
        backTo,
      ),
  },
  WORKSPACE_EXPENSIFY_CARD_EXPIRY_OPTIONS: {
    ...
    getRoute: (policyID: string, cardID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/expensify-card/${cardID}/edit/expiry-options`,
        backTo,
      ),
  },
  WORKSPACE_EXPENSIFY_CARD_ISSUE_NEW: {
    ...
    getRoute: (policyID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/expensify-card/issue-new`,
        backTo,
      ),
  },
  ...
  WORKSPACE_EXPENSIFY_CARD_BANK_ACCOUNT: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_EXPENSIFY_CARD_BANK_ACCOUNT route",
        );
      }
  ...
  WORKSPACE_EXPENSIFY_CARD_SETTINGS: {
    ...
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/expensify-card/settings` as const,
  },
  WORKSPACE_EXPENSIFY_CARD_SETTINGS_ACCOUNT: {
    ...
    getRoute: (policyID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/expensify-card/settings/account`,
        backTo,
      ),
  },
  WORKSPACE_EXPENSIFY_CARD_SELECT_FEED: {
    ...
    getRoute: (policyID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/expensify-card/select-feed`,
        backTo,
      ),
  },
  ...
  WORKSPACE_COMPANY_CARDS_SETTINGS_STATEMENT_CLOSE_DATE: {
    ...
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/company-cards/settings/statement-close-date` as const,
  },
  WORKSPACE_RULES: {
  ...
  WORKSPACE_DISTANCE_RATES: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_DISTANCE_RATES route",
        );
      }
  ...
  WORKSPACE_TRAVEL: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_TRAVEL route",
        );
      }
  ...
  WORKSPACE_TRAVEL_SETTINGS_ACCOUNT: {
    ...
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/travel/settings/account` as const,
  },
  WORKSPACE_TRAVEL_EXPORT: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_TRAVEL_EXPORT route",
        );
      }
  ...
  WORKSPACE_TRAVEL_SETTINGS_FREQUENCY: {
    ...
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/travel/settings/frequency` as const,
  },
  WORKSPACE_TRAVEL_MISSING_PERSONAL_DETAILS: {
    route: "workspaces/:policyID/travel/missing-personal-details",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/travel/missing-personal-details` as const,
  },
  ...
  WORKSPACE_PER_DIEM: {
    ...
    getRoute: (policyID: string | undefined, backTo?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_PER_DIEM route",
        );
      }
  ...
  WORKSPACE_DUPLICATE_SELECT_FEATURES: {
    ...
    getRoute: (policyID: string) =>
      `workspace/${policyID}/duplicate/select-features` as const,
  },
  WORKSPACE_RECEIPT_PARTNERS: {
    ...
    getRoute: (policyID: string | undefined, backTo?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_RECEIPT_PARTNERS route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `workspaces/${policyID}/receipt-partners`,
        backTo,
      );
    },
  },
  WORKSPACE_RECEIPT_PARTNERS_INVITE: {
    ...
    getRoute: (
      ...
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_RECEIPT_PARTNERS_INVITE route",
        );
      }
  ...
  WORKSPACE_RECEIPT_PARTNERS_CHANGE_BILLING_ACCOUNT: {
    ...
    getRoute: (policyID: string, integration: string) =>
      `workspaces/${policyID}/receipt-partners/${integration}/billing-account` as const,
  },
  WORKSPACE_RECEIPT_PARTNERS_INVITE_EDIT: {
    ...
    getRoute: (
      ...
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_RECEIPT_PARTNERS_INVITE_EDIT route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `workspaces/${policyID}/receipt-partners/${integration}/invite/edit`,
        backTo,
      );
    },
  },
  WORKSPACE_RECEIPT_PARTNERS_INVITE_EDIT_ALL:
    "workspaces/:policyID/receipt-partners/:integration/invite/edit/ReceiptPartnersAllTab",
  WORKSPACE_RECEIPT_PARTNERS_INVITE_EDIT_LINKED:
    "workspaces/:policyID/receipt-partners/:integration/invite/edit/ReceiptPartnersLinkedTab",
  WORKSPACE_RECEIPT_PARTNERS_INVITE_EDIT_OUTSTANDING:
    "workspaces/:policyID/receipt-partners/:integration/invite/edit/ReceiptPartnersOutstandingTab",

  WORKSPACE_PER_DIEM_IMPORT: {
    route: "workspaces/:policyID/per-diem/import",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/per-diem/import` as const,
  },
  WORKSPACE_PER_DIEM_IMPORTED: {
    route: "workspaces/:policyID/per-diem/imported",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/per-diem/imported` as const,
  },
  WORKSPACE_PER_DIEM_SETTINGS: {
    route: "workspaces/:policyID/per-diem/settings",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/per-diem/settings` as const,
  },
  WORKSPACE_PER_DIEM_DETAILS: {
    route: "workspaces/:policyID/per-diem/details/:rateID/:subRateID",
    getRoute: (policyID: string, rateID: string, subRateID: string) =>
      `workspaces/${policyID}/per-diem/details/${rateID}/${subRateID}` as const,
  },
  WORKSPACE_PER_DIEM_EDIT_DESTINATION: {
    route: "workspaces/:policyID/per-diem/edit/destination/:rateID/:subRateID",
    getRoute: (policyID: string, rateID: string, subRateID: string) =>
      `workspaces/${policyID}/per-diem/edit/destination/${rateID}/${subRateID}` as const,
  },
  WORKSPACE_PER_DIEM_EDIT_SUBRATE: {
    route: "workspaces/:policyID/per-diem/edit/subrate/:rateID/:subRateID",
    getRoute: (policyID: string, rateID: string, subRateID: string) =>
      `workspaces/${policyID}/per-diem/edit/subrate/${rateID}/${subRateID}` as const,
  },
  WORKSPACE_PER_DIEM_EDIT_AMOUNT: {
    route: "workspaces/:policyID/per-diem/edit/amount/:rateID/:subRateID",
    getRoute: (policyID: string, rateID: string, subRateID: string) =>
      `workspaces/${policyID}/per-diem/edit/amount/${rateID}/${subRateID}` as const,
  },
  WORKSPACE_PER_DIEM_EDIT_CURRENCY: {
    route: "workspaces/:policyID/per-diem/edit/currency/:rateID/:subRateID",
    getRoute: (policyID: string, rateID: string, subRateID: string) =>
      `workspaces/${policyID}/per-diem/edit/currency/${rateID}/${subRateID}` as const,
  },
  ...
  WORKSPACE_TIME_TRACKING: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the WORKSPACE_TIME_TRACKING route",
        );
      }
  ...
  WORKSPACE_TIME_TRACKING_DEFAULT_RATE: {
    ...
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/time-tracking/rate` as const,
  },

  REPORTS_DEFAULT_TITLE: {
    route: "workspaces/:policyID/reports/name",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/reports/name` as const,
  },
  RULES_AUTO_APPROVE_REPORTS_UNDER: {
    route: "workspaces/:policyID/rules/auto-approve",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/rules/auto-approve` as const,
  },
  RULES_RANDOM_REPORT_AUDIT: {
    route: "workspaces/:policyID/rules/audit",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/rules/audit` as const,
  },
  RULES_AUTO_PAY_REPORTS_UNDER: {
    route: "workspaces/:policyID/rules/auto-pay",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/rules/auto-pay` as const,
  },
  RULES_RECEIPT_REQUIRED_AMOUNT: {
    route: "workspaces/:policyID/rules/receipt-required-amount",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/rules/receipt-required-amount` as const,
  },
  RULES_ITEMIZED_RECEIPT_REQUIRED_AMOUNT: {
    route: "workspaces/:policyID/rules/itemized-receipt-required-amount",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/rules/itemized-receipt-required-amount` as const,
  },
  RULES_MAX_EXPENSE_AMOUNT: {
    route: "workspaces/:policyID/rules/max-expense-amount",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/rules/max-expense-amount` as const,
  },
  RULES_MAX_EXPENSE_AGE: {
    route: "workspaces/:policyID/rules/max-expense-age",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/rules/max-expense-age` as const,
  },
  RULES_BILLABLE_DEFAULT: {
    route: "workspaces/:policyID/rules/billable",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/rules/billable` as const,
  },
  RULES_REIMBURSABLE_DEFAULT: {
    route: "workspaces/:policyID/rules/reimbursable",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/rules/reimbursable` as const,
  },
  RULES_PROHIBITED_DEFAULT: {
    route: "workspaces/:policyID/rules/prohibited",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/rules/prohibited` as const,
  },
  RULES_CUSTOM: {
    route: "workspaces/:policyID/overview/policy",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/overview/policy` as const,
  },
  RULES_MERCHANT_NEW: {
    route: "workspaces/:policyID/rules/merchant-rules/new",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/rules/merchant-rules/new` as const,
  },
  RULES_MERCHANT_MERCHANT_TO_MATCH: {
    route:
      "workspaces/:policyID/rules/merchant-rules/:ruleID/merchant-to-match",
    getRoute: (policyID: string, ruleID?: string) =>
      `workspaces/${policyID}/rules/merchant-rules/${ruleID ?? "new"}/merchant-to-match` as const,
  },
  RULES_MERCHANT_MATCH_TYPE: {
    route:
      "workspaces/:policyID/rules/merchant-rules/:ruleID/merchant-to-match/type",
    getRoute: (policyID: string, ruleID?: string) =>
      `workspaces/${policyID}/rules/merchant-rules/${ruleID ?? "new"}/merchant-to-match/type` as const,
  },
  RULES_MERCHANT_MERCHANT: {
    route: "workspaces/:policyID/rules/merchant-rules/:ruleID/merchant",
    getRoute: (policyID: string, ruleID?: string) =>
      `workspaces/${policyID}/rules/merchant-rules/${ruleID ?? "new"}/merchant` as const,
  },
  RULES_MERCHANT_CATEGORY: {
    route: "workspaces/:policyID/rules/merchant-rules/:ruleID/category",
    getRoute: (policyID: string, ruleID?: string) =>
      `workspaces/${policyID}/rules/merchant-rules/${ruleID ?? "new"}/category` as const,
  },
  RULES_MERCHANT_TAG: {
    route: "workspaces/:policyID/rules/merchant-rules/:ruleID/tag/:orderWeight",
    getRoute: (policyID: string, ruleID?: string, orderWeight?: number) =>
      `workspaces/${policyID}/rules/merchant-rules/${ruleID ?? "new"}/tag/${orderWeight}` as const,
  },
  RULES_MERCHANT_TAX: {
    route: "workspaces/:policyID/rules/merchant-rules/:ruleID/tax",
    getRoute: (policyID: string, ruleID?: string) =>
      `workspaces/${policyID}/rules/merchant-rules/${ruleID ?? "new"}/tax` as const,
  },
  RULES_MERCHANT_DESCRIPTION: {
    route: "workspaces/:policyID/rules/merchant-rules/:ruleID/description",
    getRoute: (policyID: string, ruleID?: string) =>
      `workspaces/${policyID}/rules/merchant-rules/${ruleID ?? "new"}/description` as const,
  },
  RULES_MERCHANT_REIMBURSABLE: {
    route: "workspaces/:policyID/rules/merchant-rules/:ruleID/reimbursable",
    getRoute: (policyID: string, ruleID?: string) =>
      `workspaces/${policyID}/rules/merchant-rules/${ruleID ?? "new"}/reimbursable` as const,
  },
  RULES_MERCHANT_BILLABLE: {
    route: "workspaces/:policyID/rules/merchant-rules/:ruleID/billable",
    getRoute: (policyID: string, ruleID?: string) =>
      `workspaces/${policyID}/rules/merchant-rules/${ruleID ?? "new"}/billable` as const,
  },
  RULES_MERCHANT_EDIT: {
    route: "workspaces/:policyID/rules/merchant-rules/:ruleID",
    getRoute: (policyID: string, ruleID: string) =>
      `workspaces/${policyID}/rules/merchant-rules/${ruleID}` as const,
  },
  RULES_MERCHANT_PREVIEW_MATCHES: {
    route: "workspaces/:policyID/rules/merchant-rules/:ruleID/preview-matches",
    getRoute: (policyID: string, ruleID?: string) =>
      `workspaces/${policyID}/rules/merchant-rules/${ruleID ?? "new"}/preview-matches` as const,
  },
  ...
  SHARE_DETAILS: {
    ...
    getRoute: (reportOrAccountID: string) =>
      `share/share-details/${reportOrAccountID}` as const,
  },
  ...
  SHARE_SUBMIT_DETAILS: {
    ...
    getRoute: (reportOrAccountID: string) =>
      `share/submit-details/${reportOrAccountID}` as const,
  },
  CHANGE_POLICY_EDUCATIONAL: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam("change-workspace-educational", backTo),
  },
  TRAVEL_MY_TRIPS: {
    route: "travel",
    getRoute: (policyID?: string) =>
      `travel?${policyID ? `policyID=${policyID}` : ""}` as const,
  },
  TRAVEL_DOT_LINK_WEB_VIEW: {
    ...
    getRoute: (
      ...
      const redirectURL = postLoginPath
        ? `&redirectUrl=${encodeURIComponent(postLoginPath)}`
        : "";
  ...
  TRAVEL_TCS: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (domain: string, policyID?: string, backTo?: string) =>
      getUrlWithBackToParam(
        `travel/terms/${domain}/accept?${policyID ? `policyID=${policyID}` : ""}`,
        backTo,
      ),
  },
  TRAVEL_UPGRADE: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam("travel/upgrade", backTo),
  },
  ...
  TRAVEL_TRIP_SUMMARY: {
    ...
    getRoute: (
      ...
      if (!reportID || !transactionID) {
        Log.warn(
          "Invalid reportID or transactionID is used to build the TRAVEL_TRIP_SUMMARY route",
        );
      }
  ...
  TRAVEL_TRIP_DETAILS: {
    ...
    getRoute: (
      ...
      if (!reportID || !transactionID || !pnr) {
        Log.warn(
          "Invalid reportID, transactionID or pnr is used to build the TRAVEL_TRIP_DETAILS route",
        );
      }
  ...
  TRAVEL_DOMAIN_SELECTOR: {
    ...
    getRoute: (policyID?: string, backTo?: string) =>
      getUrlWithBackToParam(
        `travel/domain-selector?${policyID ? `policyID=${policyID}` : ""}`,
        backTo,
      ),
  },
  TRAVEL_DOMAIN_PERMISSION_INFO: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (domain?: string, backTo?: string) =>
      getUrlWithBackToParam(`travel/domain-permission/${domain}/info`, backTo),
  },
  TRAVEL_PUBLIC_DOMAIN_ERROR: {
    ...
    getRoute: (policyID?: string, backTo?: string) =>
      getUrlWithBackToParam(
        `travel/public-domain-error?${policyID ? `policyID=${policyID}` : ""}`,
        backTo,
      ),
  },
  TRAVEL_WORKSPACE_CONFIRMATION: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam(`travel/upgrade/workspace/confirmation`, backTo),
  },
  TRAVEL_WORKSPACE_ADDRESS: {
    ...
    getRoute: (domain: string, policyID?: string, backTo?: string) =>
      getUrlWithBackToParam(
        `travel/${domain}/workspace-address?${policyID ? `policyID=${policyID}` : ""}`,
        backTo,
      ),
  },
  TRAVEL_VERIFY_ACCOUNT: {
    ...
    getRoute: (domain?: string, policyID?: string, backTo?: string) =>
      getUrlWithBackToParam(
        getUrlWithParams(`travel/${VERIFY_ACCOUNT}`, { domain, policyID }),
        backTo,
      ),
  },
  ONBOARDING_ROOT: {
  ...
  ONBOARDING_PRIVATE_DOMAIN: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam(`onboarding/private-domain`, backTo),
  },
  ONBOARDING_EMPLOYEES: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam(`onboarding/employees`, backTo),
  },
  ONBOARDING_ACCOUNTING: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam(`onboarding/accounting`, backTo),
  },
  ONBOARDING_INTERESTED_FEATURES: {
    ...
    getRoute: (userReportedIntegration?: string, backTo?: string) =>
      getUrlWithBackToParam(
        `onboarding/interested-features?userReportedIntegration=${userReportedIntegration}`,
        backTo,
      ),
  },
  ONBOARDING_PURPOSE: {
  ...
  ONBOARDING_WORKSPACES: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam(`onboarding/join-workspaces`, backTo),
  },
  ONBOARDING_WORK_EMAIL: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam(`onboarding/work-email`, backTo),
  },
  ONBOARDING_WORK_EMAIL_VALIDATION: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam(`onboarding/work-email-validation`, backTo),
  },
  ONBOARDING_WORKSPACE: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam(`onboarding/create-workspace`, backTo),
  },
  ONBOARDING_WORKSPACE_CONFIRMATION: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam(`onboarding/workspace-confirmation`, backTo),
  },
  ONBOARDING_WORKSPACE_CURRENCY: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam(`onboarding/workspace-currency`, backTo),
  },
  CURRENCY_SELECTION: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam(`workspace/confirmation/currency`, backTo),
  },
  ONBOARDING_WORKSPACE_INVITE: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam(`onboarding/workspace-invite`, backTo),
  },
  EXPLANATION_MODAL_ROOT: "onboarding/explanation",
  TEST_DRIVE_MODAL_ROOT: {
    route: "onboarding/test-drive",
    getRoute: (bossEmail?: string) =>
      `onboarding/test-drive${bossEmail ? `?bossEmail=${encodeURIComponent(bossEmail)}` : ""}` as const,
  },
  ...
  WORKSPACE_CONFIRMATION: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam(`workspace/confirmation`, backTo),
  },
  WORKSPACE_CONFIRMATION_OWNER_SELECTOR:
    "workspace/confirmation/owner-selector",
  MIGRATED_USER_WELCOME_MODAL: {
    route: "onboarding/migrated-user-welcome",

    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam("onboarding/migrated-user-welcome", backTo, false),
  },
  ...
  TRANSACTION_RECEIPT: {
    ...
    getRoute: (
      ...
      if (!reportID) {
        Log.warn(
          "Invalid reportID is used to build the TRANSACTION_RECEIPT route",
        );
      }
      if (!transactionID) {
        Log.warn(
          "Invalid transactionID is used to build the TRANSACTION_RECEIPT route",
        );
      }
  ...
  TRANSACTION_DUPLICATE_REVIEW_PAGE: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (threadReportID: string | undefined, backTo?: string) =>
      getUrlWithBackToParam(
  ...
  TRANSACTION_DUPLICATE_REVIEW_MERCHANT_PAGE: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (threadReportID: string, backTo?: string) =>
      getUrlWithBackToParam(
  ...
  MERGE_TRANSACTION_LIST_PAGE: {
    ...
    getRoute: (transactionID: string, backTo: string, isOnSearch = false) => {
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      const url = getUrlWithBackToParam(
        `merge/${transactionID}` as const,
        backTo,
      );
  ...
  MERGE_TRANSACTION_RECEIPT_PAGE: {
    ...
    getRoute: (transactionID: string, backTo: string, isOnSearch = false) => {
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      const url = getUrlWithBackToParam(
        `merge/${transactionID}/receipt` as const,
        backTo,
      );
      return isOnSearch ? (`${url}&isOnSearch=true` as const) : url;
    },
  ...
  MERGE_TRANSACTION_DETAILS_PAGE: {
    ...
    getRoute: (transactionID: string, backTo: string, isOnSearch = false) => {
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      const url = getUrlWithBackToParam(
        `merge/${transactionID}/details` as const,
        backTo,
      );
      return isOnSearch ? (`${url}&isOnSearch=true` as const) : url;
    },
  ...
  MERGE_TRANSACTION_CONFIRMATION_PAGE: {
    ...
    getRoute: (transactionID: string, backTo: string, isOnSearch = false) => {
      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      const url = getUrlWithBackToParam(
        `merge/${transactionID}/confirmation` as const,
        backTo,
      );
      return isOnSearch ? (`${url}&isOnSearch=true` as const) : url;
    },
  ...
  POLICY_ACCOUNTING_XERO_IMPORT: {
    ...
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/xero/import` as const,
  },
  POLICY_ACCOUNTING_XERO_CHART_OF_ACCOUNTS: {
    route: "workspaces/:policyID/accounting/xero/import/accounts",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/xero/import/accounts` as const,
  },
  POLICY_ACCOUNTING_XERO_ORGANIZATION: {
    route:
      "workspaces/:policyID/accounting/xero/organization/:currentOrganizationID",
    getRoute: (
      policyID: string | undefined,
      ...
      if (!policyID || !currentOrganizationID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_XERO_ORGANIZATION route",
        );
      }
  ...
  POLICY_ACCOUNTING_XERO_TRACKING_CATEGORIES: {
    ...
    getRoute: (policyID?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_XERO_TRACKING_CATEGORIES route",
        );
      }
  ...
  POLICY_ACCOUNTING_XERO_TRACKING_CATEGORIES_MAP: {
    route:
      "workspaces/:policyID/accounting/xero/import/tracking-categories/mapping/:categoryId/:categoryName",
  ...
  POLICY_ACCOUNTING_XERO_CUSTOMER: {
    ...
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/xero/import/customers` as const,
  },
  POLICY_ACCOUNTING_XERO_TAXES: {
    route: "workspaces/:policyID/accounting/xero/import/taxes",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/xero/import/taxes` as const,
  },
  POLICY_ACCOUNTING_XERO_EXPORT: {
    ...
    getRoute: (policyID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/xero/export` as const,
        backTo,
        false,
      ),
  },
  POLICY_ACCOUNTING_XERO_PREFERRED_EXPORTER_SELECT: {
    ...
    getRoute: (policyID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/connections/xero/export/preferred-exporter/select` as const,
        backTo,
      ),
  },
  POLICY_ACCOUNTING_XERO_EXPORT_PURCHASE_BILL_DATE_SELECT: {
    ...
    getRoute: (policyID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/xero/export/purchase-bill-date-select` as const,
        backTo,
      ),
  },
  POLICY_ACCOUNTING_XERO_ADVANCED: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_XERO_ADVANCED route",
        );
      }
  ...
  POLICY_ACCOUNTING_CLAIM_OFFER: {
    ...
    getRoute: (policyID: string, integration: string) =>
      `workspaces/${policyID}/accounting/claim-offer/${integration}` as const,
  },
  POLICY_ACCOUNTING_XERO_AUTO_SYNC: {
    ...
    getRoute: (policyID: string | undefined, backTo?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_XERO_AUTO_SYNC route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/xero/advanced/autosync` as const,
        backTo,
      );
    },
  },
  POLICY_ACCOUNTING_XERO_INVOICE_SELECTOR: {
    route:
      "workspaces/:policyID/accounting/xero/advanced/invoice-account-selector",
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_XERO_INVOICE_SELECTOR route",
        );
      }
  ...
  POLICY_ACCOUNTING_XERO_BILL_PAYMENT_ACCOUNT_SELECTOR: {
    route:
      "workspaces/:policyID/accounting/xero/advanced/bill-payment-account-selector",
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_XERO_BILL_PAYMENT_ACCOUNT_SELECTOR route",
        );
      }
  ...
  POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_IMPORT: {
    ...
    getRoute: (policyID: string | undefined) =>
      `workspaces/${policyID}/accounting/quickbooks-online/import` as const,
  },
  POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_CHART_OF_ACCOUNTS: {
    ...
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/quickbooks-online/import/accounts` as const,
  },
  POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_CLASSES: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_CLASSES route",
        );
      }
  ...
  POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_CLASSES_DISPLAYED_AS: {
    route:
      "workspaces/:policyID/accounting/quickbooks-online/import/classes/displayed-as",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/quickbooks-online/import/classes/displayed-as` as const,
  },
  POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_CUSTOMERS: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_CUSTOMERS route",
        );
      }
  ...
  POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_CUSTOMERS_DISPLAYED_AS: {
    route:
      "workspaces/:policyID/accounting/quickbooks-online/import/customers/displayed-as",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/quickbooks-online/import/customers/displayed-as` as const,
  },
  POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_LOCATIONS: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_LOCATIONS route",
        );
      }
  ...
  POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_LOCATIONS_DISPLAYED_AS: {
    route:
      "workspaces/:policyID/accounting/quickbooks-online/import/locations/displayed-as",
    getRoute: (policyID: string | undefined) =>
      `workspaces/${policyID}/accounting/quickbooks-online/import/locations/displayed-as` as const,
  },
  POLICY_ACCOUNTING_QUICKBOOKS_ONLINE_TAXES: {
    route: "workspaces/:policyID/accounting/quickbooks-online/import/taxes",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/quickbooks-online/import/taxes` as const,
  },
  RESTRICTED_ACTION: {
  ...
  MISSING_PERSONAL_DETAILS_CONFIRM_MAGIC_CODE: {
    ...
    getRoute: (cardID: string) =>
      `missing-personal-details/${cardID}/confirm-magic-code` as const,
  },
  POLICY_ACCOUNTING_NETSUITE_SUBSIDIARY_SELECTOR: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_NETSUITE_SUBSIDIARY_SELECTOR route",
        );
      }
  ...
  POLICY_ACCOUNTING_NETSUITE_EXISTING_CONNECTIONS: {
    ...
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/netsuite/existing-connections` as const,
  },
  POLICY_ACCOUNTING_NETSUITE_TOKEN_INPUT: {
    ...
    getRoute: (policyID: string | undefined, subPage: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_NETSUITE_TOKEN_INPUT route",
        );
      }
  ...
  POLICY_ACCOUNTING_NETSUITE_IMPORT: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_NETSUITE_IMPORT route",
        );
      }
  ...
  POLICY_ACCOUNTING_NETSUITE_IMPORT_MAPPING: {
    route:
      "workspaces/:policyID/accounting/netsuite/import/mapping/:importField",
    getRoute: (
      policyID: string,
  ...
  POLICY_ACCOUNTING_NETSUITE_IMPORT_CUSTOM_FIELD_MAPPING: {
    route:
      "workspaces/:policyID/accounting/netsuite/import/custom/:importCustomField",
    getRoute: (
      policyID: string | undefined,
      importCustomField: ValueOf<
        typeof CONST.NETSUITE_CONFIG.IMPORT_CUSTOM_FIELDS
      >,
    ) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_NETSUITE_IMPORT_CUSTOM_FIELD_MAPPING route",
        );
      }
  ...
  POLICY_ACCOUNTING_NETSUITE_IMPORT_CUSTOM_FIELD_VIEW: {
    route:
      "workspaces/:policyID/accounting/netsuite/import/custom/:importCustomField/view/:valueIndex",
    getRoute: (
      policyID: string,
      importCustomField: ValueOf<
        typeof CONST.NETSUITE_CONFIG.IMPORT_CUSTOM_FIELDS
  ...
  POLICY_ACCOUNTING_NETSUITE_IMPORT_CUSTOM_FIELD_EDIT: {
    route:
      "workspaces/:policyID/accounting/netsuite/import/custom/:importCustomField/edit/:valueIndex/:fieldName",
    getRoute: (
      policyID: string | undefined,
      importCustomField: ValueOf<
        typeof CONST.NETSUITE_CONFIG.IMPORT_CUSTOM_FIELDS
      ...
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_NETSUITE_IMPORT_CUSTOM_FIELD_EDIT route",
        );
      }
  ...
  POLICY_ACCOUNTING_NETSUITE_IMPORT_CUSTOM_LIST_ADD: {
    route:
      "workspaces/:policyID/accounting/netsuite/import/custom-list/new/:subPage?/:action?",
    getRoute: (
      policyID: string | undefined,
      subPage?: string,
      action?: "edit",
      ...
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_NETSUITE_IMPORT_CUSTOM_LIST_ADD route",
        );
      }
      if (!subPage) {
  ...
  POLICY_ACCOUNTING_NETSUITE_IMPORT_CUSTOM_SEGMENT_ADD: {
    route:
      "workspaces/:policyID/accounting/netsuite/import/custom-segment/new/:subPage?/:action?",
    getRoute: (
      policyID: string | undefined,
      subPage?: string,
      action?: "edit",
      ...
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_NETSUITE_IMPORT_CUSTOM_SEGMENT_ADD route",
        );
      }
      if (!subPage) {
  ...
  POLICY_ACCOUNTING_NETSUITE_IMPORT_CUSTOMERS_OR_PROJECTS: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_NETSUITE_IMPORT_CUSTOMERS_OR_PROJECTS route",
        );
      }
  ...
  POLICY_ACCOUNTING_NETSUITE_IMPORT_CUSTOMERS_OR_PROJECTS_SELECT: {
    route:
      "workspaces/:policyID/accounting/netsuite/import/customer-projects/select",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/netsuite/import/customer-projects/select` as const,
  },
  POLICY_ACCOUNTING_NETSUITE_EXPORT: {
    ...
    getRoute: (policyID: string | undefined, backTo?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_NETSUITE_EXPORT route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `workspaces/${policyID}/connections/netsuite/export/` as const,
        backTo,
        false,
      );
    },
  ...
  POLICY_ACCOUNTING_NETSUITE_PREFERRED_EXPORTER_SELECT: {
    route:
      "workspaces/:policyID/connections/netsuite/export/preferred-exporter/select",

    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (policyID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/connections/netsuite/export/preferred-exporter/select` as const,
        backTo,
      ),
  },
  POLICY_ACCOUNTING_NETSUITE_DATE_SELECT: {
    ...
    getRoute: (policyID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/connections/netsuite/export/date/select` as const,
        backTo,
      ),
  },
  POLICY_ACCOUNTING_NETSUITE_EXPORT_EXPENSES: {
    ...
    getRoute: (
      ...
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_NETSUITE_EXPORT_EXPENSES route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `workspaces/${policyID}/connections/netsuite/export/expenses/${expenseType as string}` as const,
        backTo,
      );
    },
  },
  POLICY_ACCOUNTING_NETSUITE_EXPORT_EXPENSES_DESTINATION_SELECT: {
    route:
      "workspaces/:policyID/connections/netsuite/export/expenses/:expenseType/destination/select",
    getRoute: (
      policyID: string,
      expenseType: ValueOf<typeof CONST.NETSUITE_EXPENSE_TYPE>,
      backTo?: string,
  ...
  POLICY_ACCOUNTING_NETSUITE_INVOICE_ITEM_PREFERENCE_SELECT: {
    ...
    getRoute: (policyID: string | undefined, backTo?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_NETSUITE_INVOICE_ITEM_PREFERENCE_SELECT route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `workspaces/${policyID}/connections/netsuite/export/invoice-item-preference/select` as const,
        backTo,
      );
    },
  },
  POLICY_ACCOUNTING_NETSUITE_INVOICE_ITEM_SELECT: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_NETSUITE_INVOICE_ITEM_SELECT route",
        );
      }
  ...
  POLICY_ACCOUNTING_NETSUITE_TAX_POSTING_ACCOUNT_SELECT: {
    route:
      "workspaces/:policyID/connections/netsuite/export/tax-posting-account/select",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/connections/netsuite/export/tax-posting-account/select` as const,
  },
  POLICY_ACCOUNTING_NETSUITE_PROVINCIAL_TAX_POSTING_ACCOUNT_SELECT: {
    route:
      "workspaces/:policyID/connections/netsuite/export/provincial-tax-posting-account/select",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/connections/netsuite/export/provincial-tax-posting-account/select` as const,
  },
  POLICY_ACCOUNTING_NETSUITE_ADVANCED: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_NETSUITE_ADVANCED route",
        );
      }
  ...
  POLICY_ACCOUNTING_NETSUITE_REIMBURSEMENT_ACCOUNT_SELECT: {
    route:
      "workspaces/:policyID/connections/netsuite/advanced/reimbursement-account/select",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/connections/netsuite/advanced/reimbursement-account/select` as const,
  },
  POLICY_ACCOUNTING_NETSUITE_COLLECTION_ACCOUNT_SELECT: {
    route:
      "workspaces/:policyID/connections/netsuite/advanced/collection-account/select",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/connections/netsuite/advanced/collection-account/select` as const,
  },
  POLICY_ACCOUNTING_NETSUITE_EXPENSE_REPORT_APPROVAL_LEVEL_SELECT: {
    route:
      "workspaces/:policyID/connections/netsuite/advanced/expense-report-approval-level/select",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/connections/netsuite/advanced/expense-report-approval-level/select` as const,
  },
  POLICY_ACCOUNTING_NETSUITE_VENDOR_BILL_APPROVAL_LEVEL_SELECT: {
    route:
      "workspaces/:policyID/connections/netsuite/advanced/vendor-bill-approval-level/select",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/connections/netsuite/advanced/vendor-bill-approval-level/select` as const,
  },
  POLICY_ACCOUNTING_NETSUITE_JOURNAL_ENTRY_APPROVAL_LEVEL_SELECT: {
    route:
      "workspaces/:policyID/connections/netsuite/advanced/journal-entry-approval-level/select",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/connections/netsuite/advanced/journal-entry-approval-level/select` as const,
  },
  POLICY_ACCOUNTING_NETSUITE_APPROVAL_ACCOUNT_SELECT: {
    route:
      "workspaces/:policyID/connections/netsuite/advanced/approval-account/select",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/connections/netsuite/advanced/approval-account/select` as const,
  },
  POLICY_ACCOUNTING_NETSUITE_CUSTOM_FORM_ID: {
    route:
      "workspaces/:policyID/connections/netsuite/advanced/custom-form-id/:expenseType",
    getRoute: (
      policyID: string,
      expenseType: ValueOf<typeof CONST.NETSUITE_EXPENSE_TYPE>,
    ) =>
  ...
  POLICY_ACCOUNTING_NETSUITE_AUTO_SYNC: {
    ...
    getRoute: (policyID: string | undefined, backTo?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_NETSUITE_AUTO_SYNC route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `workspaces/${policyID}/connections/netsuite/advanced/autosync` as const,
        backTo,
      );
    },
  },
  POLICY_ACCOUNTING_NETSUITE_ACCOUNTING_METHOD: {
    ...
    getRoute: (policyID: string | undefined, backTo?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_NETSUITE_ACCOUNTING_METHOD route",
        );
      }

      // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
      return getUrlWithBackToParam(
        `workspaces/${policyID}/connections/netsuite/advanced/autosync/accounting-method` as const,
        backTo,
      );
    },
  },
  POLICY_ACCOUNTING_SAGE_INTACCT_PREREQUISITES: {
    ...
    getRoute: (policyID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/sage-intacct/prerequisites` as const,
        backTo,
      ),
  },
  POLICY_ACCOUNTING_SAGE_INTACCT_ENTER_CREDENTIALS: {
    ...
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/sage-intacct/enter-credentials` as const,
  },
  POLICY_ACCOUNTING_SAGE_INTACCT_EXISTING_CONNECTIONS: {
    ...
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/sage-intacct/existing-connections` as const,
  },
  POLICY_ACCOUNTING_SAGE_INTACCT_ENTITY: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_SAGE_INTACCT_ENTITY route",
        );
      }
  ...
  POLICY_ACCOUNTING_SAGE_INTACCT_IMPORT: {
    ...
    getRoute: (policyID: string | undefined) =>
      `workspaces/${policyID}/accounting/sage-intacct/import` as const,
  },
  POLICY_ACCOUNTING_SAGE_INTACCT_TOGGLE_MAPPINGS: {
    route:
      "workspaces/:policyID/accounting/sage-intacct/import/toggle-mapping/:mapping",
    getRoute: (policyID: string | undefined, mapping: SageIntacctMappingName) =>
      `workspaces/${policyID}/accounting/sage-intacct/import/toggle-mapping/${mapping as string}` as const,
  },
  POLICY_ACCOUNTING_SAGE_INTACCT_MAPPINGS_TYPE: {
    route:
      "workspaces/:policyID/accounting/sage-intacct/import/mapping-type/:mapping",
    getRoute: (policyID: string | undefined, mapping: string) =>
      `workspaces/${policyID}/accounting/sage-intacct/import/mapping-type/${mapping}` as const,
  },
  POLICY_ACCOUNTING_SAGE_INTACCT_IMPORT_TAX: {
    route: "workspaces/:policyID/accounting/sage-intacct/import/tax",
    getRoute: (policyID: string | undefined) =>
      `workspaces/${policyID}/accounting/sage-intacct/import/tax` as const,
  },
  POLICY_ACCOUNTING_SAGE_INTACCT_IMPORT_TAX_MAPPING: {
    route: "workspaces/:policyID/accounting/sage-intacct/import/tax/mapping",
    getRoute: (policyID: string | undefined) =>
      `workspaces/${policyID}/accounting/sage-intacct/import/tax/mapping` as const,
  },
  POLICY_ACCOUNTING_SAGE_INTACCT_USER_DIMENSIONS: {
    route:
      "workspaces/:policyID/accounting/sage-intacct/import/user-dimensions",
    getRoute: (policyID: string | undefined) =>
      `workspaces/${policyID}/accounting/sage-intacct/import/user-dimensions` as const,
  },
  POLICY_ACCOUNTING_SAGE_INTACCT_ADD_USER_DIMENSION: {
    route:
      "workspaces/:policyID/accounting/sage-intacct/import/add-user-dimension",
    getRoute: (policyID: string) =>
      `workspaces/${policyID}/accounting/sage-intacct/import/add-user-dimension` as const,
  },
  POLICY_ACCOUNTING_SAGE_INTACCT_EDIT_USER_DIMENSION: {
    route:
      "workspaces/:policyID/accounting/sage-intacct/import/edit-user-dimension/:dimensionName",
    getRoute: (policyID: string, dimensionName: string) =>
      `workspaces/${policyID}/accounting/sage-intacct/import/edit-user-dimension/${dimensionName}` as const,
  },
  POLICY_ACCOUNTING_SAGE_INTACCT_EXPORT: {
    ...
    getRoute: (policyID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/sage-intacct/export` as const,
        backTo,
        false,
      ),
  },
  POLICY_ACCOUNTING_SAGE_INTACCT_PREFERRED_EXPORTER: {
    route:
      "workspaces/:policyID/accounting/sage-intacct/export/preferred-exporter",

    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (policyID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/sage-intacct/export/preferred-exporter` as const,
        backTo,
      ),
  },
  POLICY_ACCOUNTING_SAGE_INTACCT_EXPORT_DATE: {
    ...
    getRoute: (policyID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/sage-intacct/export/date` as const,
        backTo,
      ),
  },
  POLICY_ACCOUNTING_SAGE_INTACCT_REIMBURSABLE_EXPENSES: {
    ...
    getRoute: (policyID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/sage-intacct/export/reimbursable` as const,
        backTo,
      ),
  },
  POLICY_ACCOUNTING_SAGE_INTACCT_NON_REIMBURSABLE_EXPENSES: {
    route:
      "workspaces/:policyID/accounting/sage-intacct/export/nonreimbursable",

    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (policyID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/sage-intacct/export/nonreimbursable` as const,
        backTo,
      ),
  },
  POLICY_ACCOUNTING_SAGE_INTACCT_REIMBURSABLE_DESTINATION: {
    route:
      "workspaces/:policyID/accounting/sage-intacct/export/reimbursable/destination",

    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (policyID: string, backTo?: string) =>
      getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/sage-intacct/export/reimbursable/destination` as const,
        backTo,
      ),
  },
  POLICY_ACCOUNTING_SAGE_INTACCT_ADVANCED: {
    ...
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_SAGE_INTACCT_ADVANCED route",
        );
      }
  ...
  POLICY_ACCOUNTING_SAGE_INTACCT_PAYMENT_ACCOUNT: {
    route:
      "workspaces/:policyID/accounting/sage-intacct/advanced/payment-account",
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_SAGE_INTACCT_PAYMENT_ACCOUNT route",
        );
      }
  ...
  POLICY_ACCOUNTING_SAGE_INTACCT_AUTO_SYNC: {
    ...
    getRoute: (policyID: string | undefined, backTo?: string) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_SAGE_INTACCT_AUTO_SYNC route",
        );
      }
      // eslint-disable-next-line no-restricted-syntax
      return getUrlWithBackToParam(
        `workspaces/${policyID}/accounting/sage-intacct/advanced/autosync` as const,
        backTo,
      );
    },
  },
  POLICY_ACCOUNTING_SAGE_INTACCT_ACCOUNTING_METHOD: {
    route:
      "workspaces/:policyID/accounting/sage-intacct/advanced/autosync/accounting-method",
    getRoute: (policyID: string | undefined) => {
      if (!policyID) {
        Log.warn(
          "Invalid policyID is used to build the POLICY_ACCOUNTING_SAGE_INTACCT_ACCOUNTING_METHOD route",
        );
      }
  ...
  ADD_UNREPORTED_EXPENSE: {
    ...
    getRoute: (reportID: string | undefined, backToReport?: string) =>
      `search/r/${reportID}/add-unreported-expense/${backToReport ?? ""}` as const,
  },
  DEBUG_REPORT: {
    route: "debug/report/:reportID",
    getRoute: (reportID: string | undefined) =>
      `debug/report/${reportID}` as const,
  },
  ...
  DEBUG_REPORT_ACTION: {
    ...
    getRoute: (reportID: string | undefined, reportActionID: string) => {
      if (!reportID) {
        Log.warn(
          "Invalid reportID is used to build the DEBUG_REPORT_ACTION route",
        );
      }
  ...
  DEBUG_REPORT_ACTION_CREATE: {
    ...
    getRoute: (reportID: string) =>
      `debug/report/${reportID}/actions/create` as const,
  },
  ...
  SCHEDULE_CALL_CONFIRMATION: {
    ...
    getRoute: (reportID: string) =>
      `r/${reportID}/schedule-call/confirmation` as const,
  },
  ...
  TEST_TOOLS_MODAL: {
    ...
    // eslint-disable-next-line no-restricted-syntax -- Legacy route generation
    getRoute: (backTo?: string) =>
      getUrlWithBackToParam("test-tools" as const, backTo),
  },
  WORKSPACES_VERIFY_DOMAIN: {
    route: "workspaces/verify-domain/:domainAccountID",
    getRoute: (domainAccountID: number) =>
      `workspaces/verify-domain/${domainAccountID}` as const,
  },
  WORKSPACES_DOMAIN_VERIFIED: {
    route: "workspaces/domain-verified/:domainAccountID",
    getRoute: (domainAccountID: number) =>
      `workspaces/domain-verified/${domainAccountID}` as const,
  },
  ...
  WORKSPACES_DOMAIN_ADDED: {
    ...
    getRoute: (domainAccountID: number) =>
      `workspaces/domain-added/${domainAccountID}` as const,
  },
  WORKSPACES_DOMAIN_ACCESS_RESTRICTED: {
    route: "workspaces/domain-access-restricted/:domainAccountID",
    getRoute: (domainAccountID: number) =>
      `workspaces/domain-access-restricted/${domainAccountID}` as const,
  },
  DOMAIN_INITIAL: {
  ...
  MULTIFACTOR_AUTHENTICATION_OUTCOME_FAILURE:
    "multifactor-authentication/outcome/failure",

  MULTIFACTOR_AUTHENTICATION_PROMPT: {
    route: `multifactor-authentication/prompt/:promptType`,
    getRoute: (promptType: MultifactorAuthenticationPromptType) =>
      `multifactor-authentication/prompt/${promptType}` as const,
  },
...
type ReportAttachmentsRouteParams =
  RootNavigatorParamList[typeof SCREENS.REPORT_ATTACHMENTS];
type ReportAddAttachmentRouteParams =
  RootNavigatorParamList[typeof SCREENS.REPORT_ADD_ATTACHMENT];

function getAttachmentModalScreenRoute(
  url: AttachmentRoutes,
  params?: ReportAttachmentsRouteParams | ReportAddAttachmentRouteParams,
) {
  if (!params?.source) {
    return url;
  }
  ...
  const reportActionIDParam = reportActionID
  ...
  const fileNameParam = originalFileName
    ...
    : "";
  const attachmentLinkParam = attachmentLink
...
type DynamicRouteSuffix =
  (typeof DYNAMIC_ROUTES)[keyof typeof DYNAMIC_ROUTES]["path"];
